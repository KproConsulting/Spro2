...
<form name="ConvertLead" method="POST" action="index.php" onsubmit="VtigerJS_DialogBox.block();">
	...
	<div id="orgLay2" style="display: block; border: 1; width: 500px;" class="layerPopup crmvDiv">
		...
		<table border=0 cellspacing=0 cellpadding=5 width=100% align=center>
			...
			<tr style='display: none;' > {* kpro@tom250520171530 *} {* kpro@tom150620181140 migrazione vte18.05 *}
				<td class="small">
					<table border="0" cellspacing="0" cellpadding="0" width="100%" align="center" bgcolor="white">
						<tr>
							<td colspan="4" class="detailedViewHeader">
									...
									<table border="0" cellspacing="0" cellpadding="5" width="100%" align="center" bgcolor="white">
												...
												<div class="{if $mandatory}dvtCellInfoOff{else}dvtCellInfo{/if}">
													...
													<table cellspacing="0" cellpadding="0" width="100%">
														<tr>
															...
															<td>
																<input name="closingdate" {if $mandatory}record="true"{/if} module="Potentials" style="border: 1px solid rgb(186, 186, 186);" id="jscal_field_closedate" type="text" tabindex="4" size="10" maxlength="10" value="{$UIINFO->getMappedFieldValue('Potentials','closingdate',1)}">
															</td>
															<td>
										...
										{if $UIINFO->isActive('amount','Potentials')}
											<td width="50%">
												...
												<div class="{if $mandatory}dvtCellInfoOff{else}dvtCellInfo{/if}">
													<input type="text" name="amount" class="detailedViewTextBox" {if $mandatory}record="true"{/if} module="Potentials" value="{$UIINFO->getMappedFieldValue('Potentials','amount',1)}"></input>
												</div>
											</td>
										{/if}
										</tr>
			...
			<tr style='display: none;' > {* kpro@tom250520171530 *} {* kpro@tom150620181140 migrazione vte18.05 *}
				<td class="small">
					<table border="0" cellspacing="0" cellpadding="0" width="100%" align="center" bgcolor="white">
						<tr>
							<td colspan="4" class="detailedViewHeader">
								<input type="checkbox" onclick="javascript:showHideStatus('contact_block',null,null);" id="select_contact" name="entities[]" value="Contacts"></input>	{* kpro@tom250520171530 *} {* kpro@tom150620181140 migrazione vte18.05 *}
				...
				<td style="border-top:1px solid #DEDEDE;">
					<table border="0" cellspacing="0" cellpadding="5" width="100%" align="center" bgcolor="white">
						<tr valign="top">
							...
							<td width="50%">
								{* {include file="FieldHeader.tpl" label='LBL_TRANSFER_RELATED_RECORDS_TO'|@getTranslatedString:$MODULE} *}	{* kpro@tom250520171530 *} {* kpro@tom150620181140 migrazione vte18.05 *}
								<div style='display: none;'>	{* kpro@tom250520171530 *} {* kpro@tom150620181140 migrazione vte18.05 *}
									{if $UIINFO->isModuleActive('Accounts') eq true && $row.company neq ''}<input type="radio" name="transferto" id="transfertoacc" value="Accounts" checked="checked" onclick="selectTransferTo('Accounts')" {if $UIINFO->isModuleActive('Contacts') neq true}checked="checked"{/if} /><label for="transfertoacc">{'SINGLE_Accounts'|@getTranslatedString:$MODULE}</label>{/if}	{* kpro@tom250520171530 *} {* kpro@tom150620181140 migrazione vte18.05 *}
									{if $UIINFO->isModuleActive('Contacts') eq true}<input type="radio" name="transferto" id="transfertocon" value="Contacts" onclick="selectTransferTo('Contacts')"  /><label for="transfertocon">{'SINGLE_Contacts'|@getTranslatedString:$MODULE}</label>{/if}	{* kpro@tom250520171530 *} {* kpro@tom150620181140 migrazione vte18.05 *}
								</div>
							</td>
						</tr>
					</table>
...
<script type="text/javascript" id="drag_conv_leadcal">
	var CLHandle = document.getElementById("ConvertLead_Handle");
	var CLRoot   = document.getElementById("convertleaddiv");
	Drag.init(CLHandle, CLRoot);
</script>