					...
					<div {if $fld_displayvalue|trim eq ''}class="dvtCellInfo"{else}class="dvtCellInfoOff"{/if} style="position:relative"> {* crmv@97216 *}
						...
						<div class="dvtCellInfoImgRx">
							{if $entityType eq 'Services'}
								<img id="searchIcon{$row_no}" title="Services" src="{'themes/images/modulesimg/Services.png'|resourcever}" style="cursor: pointer;" align="absmiddle" onclick="servicePickList(this,'{$MODULE}','{$row_no}')" width="23" />
							{else}
								<img id="searchIcon{$row_no}" title="Products" src="{'InventoryProducts.png'|resourcever}" style="cursor: pointer;" align="absmiddle" onclick="productPickList(this,'{$MODULE}','{$row_no}')" width="23" />
							{/if}
						</div>
			...
			<tr>
				<td class="small" id="setComment">
					<div class="dvtCellInfo" style="position:relative"> {* crmv@97216 *}
						...
						<div class="dvtCellInfoImgRx">
							<img src="{'clear_field.gif'|resourcever}" onClick="{literal}${/literal}('{$comment}').value=''"; style="cursor:pointer;" />
						</div>
					</div>
				</td>
			</tr>
			...
			<tr>
				<td>
					<div class="dvtCellInfo">
						...
						<input id="{$qty}" name="{$qty}" type="text" class="detailedViewTextBox" onChange="settotalnoofrows(); calcTotal(); setDiscount(this,'{$row_no}'); callTaxCalc('{$row_no}'); calcTotal(); {if $MODULE eq 'Invoice' && $entityType neq 'Services'} stock_alert('{$row_no}');{/if}" value="{$data.$qty|formatUserNumber}"/><br><span id="stock_alert{$row_no}"></span> <!-- kpro@bid291120171400 spostato setDiscount(this,'{$row_no}'); dopo il primo calcTotal(); e aggiunto alla fine callTaxCalc('{$row_no}'); calcTotal(); -->
		   ...
		   <tr>
			<td align="right">
				<div class="dvtCellInfo" style="position:relative"> {* crmv@97216 *}
					...
					<input id="{$listPrice}" name="{$listPrice}" value="{$data.$listPrice|formatUserNumber}" type="text" class="detailedViewTextBox" onChange="calcTotal(); setDiscount(this,'{$row_no}');callTaxCalc('{$row_no}'); calcTotal();"/> <!-- kpro@bid291120171400 aggiunto alla fine calcTotal(); -->
					<!-- kpro@tom150620181140 migrazione vte18.05 end -->

					<div class="dvtCellInfoImgRx">
						<img src="{'pricebook.gif'|resourcever}" onclick="priceBookPickList(this,'{$row_no}')" align="absmiddle" style="cursor:pointer">
					</div>
				</div>
			</td>
		   </tr>
				...
				<div class="discountUI" id="discount_div{$row_no}">
					...
					<table width="100%" border="0" cellpadding="5" cellspacing="0" class="small">
					   ...
					   <tr>
						...
						<td align="right"><input type="text" class="small" size="8" id="discount_percentage{$row_no}" name="discount_percentage{$row_no}" value="{$data.$discount_percent}" {$data.$style_discount_percent} onChange="setDiscount(this,'{$row_no}'); callTaxCalc('{$row_no}'); calcTotal();">&nbsp;%</td>
					   </tr>
					   <tr>
						...
						<td align="right"><input type="text" class="small" size="8" id="discount_amount{$row_no}" name="discount_amount{$row_no}" value="{$data.$discount_amount|formatUserNumber}" {$data.$style_discount_amount} onChange="setDiscount(this,{$row_no}); callTaxCalc('{$row_no}'); calcTotal();">&nbsp;&nbsp;&nbsp;&nbsp;</td>
					   </tr>
					   {* crmv@2539me *}
					</table>
