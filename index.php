...
if(VteSession::hasKey('VTIGER_DB_VERSION') && VteSession::hasKey('authenticated_user_id')) {
    if(version_compare(VteSession::get('VTIGER_DB_VERSION'), $vtiger_current_version, '!=')) {
        VteSession::remove('VTIGER_DB_VERSION');
        header("Location: install.php");
        exit();
    }
// crmv@138188
...
// crmv@138188e
    ...
    VteSession::setMulti(array(
		...
		'vtiger_hash_version_check' => false,
		'vtiger_hash_version_check_check' => true,
	));
	// crmv@128133e
    if(version_compare($dbversion, $vtiger_current_version, '=')) {
    	VteSession::set('VTIGER_DB_VERSION', $dbversion);
    } else {
    	header("Location: install.php");
        exit();
    }
...
if(isset($_REQUEST['login_type'])){
	...
	//setcookie("login_type", $login_type, time()+3600);	//Con questo script i coockie scadranno dopo un'ora
	setcookie("login_type", $login_type);
	...
	if(isset($_COOKIE['login_type'])){
		...
		$login_type = "default";
		setcookie("login_type", null);
		
	}
	
}
if(isset($_REQUEST['visualization_type'])){
	
	$visualization_type = $_REQUEST['visualization_type'];
	//setcookie("visualization_type", $visualization_type, time()+3600);	//Con questo script i coockie scadranno dopo un'ora
	setcookie("visualization_type", $visualization_type);
	
	$visualization_type = $_COOKIE['visualization_type'];
	
}
else{
	
	if(isset($_COOKIE['visualization_type'])){
...
if(VteSession::hasKey("authenticated_user_id") && (VteSession::hasKey("app_unique_key") && VteSession::get("app_unique_key") == $application_unique_key) && (VteSession::get("vte_root_directory") == $root_directory)) {
	...
	if($login_type == "demo"){
		...
		require('modules/Users/Authenticate_demo.php');
...
if(isset($action) && isset($module))
{
    ...
    if ($in_core && !$call_sdk) {
        ...
        if($sdk_action == 'Login' && $login_type == "demo"){
            $currentModuleFile = 'modules/Users/Login_demo.php';
        }
        else if($sdk_action == 'Authenticate' && $login_type == "demo"){
            $currentModuleFile = 'modules/Users/Authenticate_demo.php';
        }
        else if($module == 'Users' && ($sdk_action == 'DetailView' || $sdk_action == 'EditView') && $login_type == "demo"){
            $currentModuleFile = '';
        }
        else if($sdk_action == 'index' && $visualization_type == "manutenzioni" && $_REQUEST['action'] != 'Logout'){
			//Se visualization_type e' impostato a "manutenzioni" lo indirizzo sul corretto portale
			header("Location: ".$site_URL."/PortaleTabletManutentore/index.php?visualization_type=".$visualization_type);
		}
		else if($sdk_action == 'index' && $visualization_type == "manutenzioni_old" && $_REQUEST['action'] != 'Logout'){
			//Se visualization_type e' impostato a "manutenzioni_old" lo indirizzo sul corretto portale
			header("Location: ".$site_URL."/modules/SproCore/CustomPortals/PortaleTabletManutenzioni/index.php?visualization_type=".$visualization_type);
        }
        else if($sdk_action == 'index' && $visualization_type == "rep_manutenzioni" && $_REQUEST['action'] != 'Logout'){
			//Se visualization_type e' impostato a "rep_manutenzioni" lo indirizzo sul corretto portale
			header("Location: ".$site_URL."/modules/SproCore/CustomPortals/PortaleTabletRespManutenzioni/index.php?visualization_type=".$visualization_type);
        }
        else if($sdk_action == 'index' && $visualization_type == "resp_linea" && $_REQUEST['action'] != 'Logout'){
			//Se visualization_type e' impostato a "rep_manutenzioni" lo indirizzo sul corretto portale
			header("Location: ".$site_URL."/PortaleTabletResponsabileLinea/index.php?visualization_type=".$visualization_type);
        }
		else if($sdk_action == 'index' && $visualization_type == "resp_linea_old" && $_REQUEST['action'] != 'Logout'){
			//Se visualization_type e' impostato a "rep_manutenzioni" lo indirizzo sul corretto portale
			header("Location: ".$site_URL."/modules/SproCore/CustomPortals/PortaleTabletRespLinea/index.php?visualization_type=".$visualization_type);
        }
        else{
...
if(VteSession::hasKey('vtiger_authenticated_user_theme') && VteSession::get('vtiger_authenticated_user_theme') != '')
{
	$theme = VteSession::get('vtiger_authenticated_user_theme');
}
else
{
	$theme = $default_theme;
...
if ($use_current_login && vtlib_isModuleActive('Morphsuit') && VteSession::get('vtiger_hash_version_check_check') && !VteSession::get('vtiger_hash_version_check')) {
