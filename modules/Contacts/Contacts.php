	...
	*/
	function get_faxes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT case when (".$table_prefix."_users.user_name is not null) then ".$table_prefix."_users.user_name else ".$table_prefix."_groups.groupname end as user_name,
			...
			FROM ".$table_prefix."_activity, ".$table_prefix."_seactivityrel, ".$table_prefix."_contactdetails, ".$table_prefix."_users, ".$table_prefix."_crmentity
			...
			WHERE ".$table_prefix."_seactivityrel.activityid = ".$table_prefix."_activity.activityid
				AND ".$table_prefix."_contactdetails.contactid = ".$table_prefix."_seactivityrel.crmid
				...
				AND ".$table_prefix."_contactdetails.contactid = ".$id."
	...
	*/
	function get_sms($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT case when (".$table_prefix."_users.user_name is not null) then ".$table_prefix."_users.user_name else ".$table_prefix."_groups.groupname end as user_name,
			...
			".$table_prefix."_activity.activitytype, ".$table_prefix."_crmentity.modifiedtime,
			".$table_prefix."_crmentity.crmid, ".$table_prefix."_crmentity.smownerid, ".$table_prefix."_activity.date_start, ".$table_prefix."_seactivityrel.crmid as parent_id,
			".$table_prefix."_activity.description
			FROM ".$table_prefix."_activity, ".$table_prefix."_seactivityrel, ".$table_prefix."_contactdetails, ".$table_prefix."_users, ".$table_prefix."_crmentity
			LEFT JOIN ".$table_prefix."_groups
				ON ".$table_prefix."_groups.groupid=".$table_prefix."_crmentity.smownerid
			WHERE ".$table_prefix."_seactivityrel.activityid = ".$table_prefix."_activity.activityid
	...
	*/
	...
	function syncPortalInfo() {
		...
		*/
		...
		$query = "SELECT vendor_id FROM {$table_prefix}_contactdetails WHERE contactid = ".$this->id;
		...
		if ($sendMail) {
			...
			if($fornitore != null && $fornitore != "" && $fornitore != 0){
				$this->kpSendPortalFornitoreEmail($email, $email, $password);
			}
			else{
				$this->sendPortalEmail($email, $email, $password);
			}
	...
	/* kpro@tom150620181140 migrazione vte18.05 */
	function kpSendPortalFornitoreEmail($email, $username, $password) {
		global $PORTAL_URL, $current_user, $site_URL;
		...
		$data_array['portal_url'] = '<a href="'.$site_URL.'/KpPortaleConsulente" style="font-family:Arial, Helvetica, sans-serif;font-size:12px; font-weight:bolder;text-decoration:none;color: #4242FD;">'.getTranslatedString('Please Login Here', 'Contacts').'</a>';
	
		$value = getmail_contents_portalUser($data_array,$password,"LoginDetails");
		$contents = $value["body"];
		//$subject = $value["subject"];
		$subject = "Dettagli di Accesso al Portale Consulenti";
	...
	*/
	function insertIntoAttachment($id,$module)
	{
		...
		if($module == 'Contacts' && $file_saved)
		{
			if($old_attachmentid != '')
			{
				$setype = $adb->query_result($adb->pquery("select setype from ".$table_prefix."_crmentity where crmid=?", array($old_attachmentid)),0,'setype');
