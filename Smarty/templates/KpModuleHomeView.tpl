				...
				<div class="pull-left" style="position:relative">
					...
					{foreach item=VIEW from=$MODHOMEVIEWS}
						...
						<td class="{$_class}" style="padding-right:5px" align="center" id="tdViewTab_{$VIEW.modhomeid}" nowrap="">
							...
							{* kpro@bid160620170950 *}
							{assign var="IS_KP_MODULEHOME" value=false}
							{if $KP_LIST_VIEW_TAB} 
								{foreach item=KP_VIEW from=$KP_LIST_VIEW_TAB_DETAILS}
									{if $VIEW.modhomeid == $KP_VIEW.modhomeid}
										{assign var="IS_KP_MODULEHOME" value=true}
									{/if}
								{/foreach}
							{/if}
							{* kpro@bid160620170950 end *}

							{if !$IS_KP_MODULEHOME} {* kpro@bid160620170950 *}
								...
								{/if}
							{else} {* kpro@bid160620170950 *}
								<span style="width:20px;height:18px;display:inline-block;vertical-align:text-top">
									
								</span>
							{/if} {* kpro@bid160620170950 end *}
	...
	{foreach item=VIEW from=$MODHOMEVIEWS}
		...
		{if $KP_LIST_VIEW_TAB} 
			{foreach item=KP_VIEW from=$KP_LIST_VIEW_TAB_DETAILS}
				{if $VIEW.modhomeid == $KP_VIEW.modhomeid}
					{assign var="IS_KP_MODULEHOME" value=true}
					<div id="MainMatrix" class="topMarginHomepage" style="padding:0px;width:100%;margin-left:0px;">
						{include file="../../`$KP_VIEW.src`" KP_MODHOMEID=$KP_VIEW.modhomeid KP_KPMODHOMEID=$KP_VIEW.kpmodhomeid KP_TABID=$KP_VIEW.tabid KP_NAME=$KP_VIEW.name}
					</div>
				{/if}
			{/foreach}
		{/if}
