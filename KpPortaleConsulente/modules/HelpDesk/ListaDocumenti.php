...
if(isset($_GET['record'])){
	...
	if(isset($_GET['annullato'])){
        $search_annullato = htmlspecialchars(addslashes(html_entity_decode(strip_tags($_GET['annullato']), ENT_QUOTES,$default_charset)), ENT_QUOTES,$default_charset);
		$search_annullato = substr($search_annullato, 0, 100);
    }
    else{
        $search_annullato = '';
    }
	...
	$q_documenti = "SELECT attac.attachmentsid attachmentsid, 
					...
					notes.kp_annullato_portal kp_annullato_portal,
					...
					WHERE ent.deleted = 0 AND notes.active_portal = 1 AND notes.folderid = 48 AND senote.crmid = ".$record;
	...
	if($search_annullato == "Si"){
		$q_documenti .= " AND notes.kp_annullato_portal = '1'";
	}
	else{
		$q_documenti .= " AND (notes.kp_annullato_portal = '0' OR notes.kp_annullato_portal = '' OR notes.kp_annullato_portal IS NULL)";
	}
	...
	for($i=0; $i < $num_documenti; $i++){
		...
		if($search_data == "" || ($search_data != "" && $data_documento == $search_data)){
			...
			$annullato_portal = $adb->query_result($res_documenti, $i, 'kp_annullato_portal');
			$annullato_portal = html_entity_decode(strip_tags($annullato_portal), ENT_QUOTES,$default_charset);
			if($annullato_portal == '1'){
				$annullato_portal = "Sì";
			}
			else{
				$annullato_portal = "No";
			...
			$rows[] = array('notesid' => $notesid,
							...
							'annullato_portal' => $annullato_portal,
							'tipo' => $tipo_download);
		}
