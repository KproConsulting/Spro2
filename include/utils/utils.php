...
function to_html($string, $encode=true)
{
	...
	if(is_string($string))
	{
		if($action != 'CustomView' && $action != 'Export' && $action != $ajax_action && $action != 'LeadConvertToEntities' && $action != 'CreatePDF' && $action != 'CreatePDFFromTemplate' && $action != 'ConvertAsFAQ' && $_REQUEST['module'] != 'Dashboard' && $action != 'CreateSOPDF' && $action != 'SendPDFMail' && (!isset($_REQUEST['submode'])) ) // crmv@104506
		{
			$doconvert = true;
		}
		else if($search == true)
		...
		// crmv@124091
		if ($_REQUEST['ajaxCall'] == 'CalendarView') {
			$doconvert = true;
		}
		// crmv@124091e
...
function getTabname($tabid) {
	...
	$name = getTabModuleName($tabid);
	// maybe it's not active, try the query
	if (empty($name)) {
...
function getUserEmail($userid)
{
	...
	$log->info("in getUserEmail ".$userid);
        ...
        if($userid != '')
        {
                $sql = "select email1 from ".$table_prefix."_users where id=?";
...
function getProfile2AllFieldList($mod_array,$profileid)
{
	...
	$profilelist=array();
	//mycrmv@164436
	/* kpro@bid290820181600 correzioni per php7 */
	...
	foreach($mod_array as $k=>$v)
	{
		...
		//next($mod_array);
		/* kpro@bid290820181600 correzioni per php7 end */
		//mycrmv@164436e
	}
...
function html_to_utf8 ($data) {
	return preg_replace_callback("/\\&\\#([0-9]{3,10})\\;/", create_function('$matches','return _html_to_utf8($matches[1]);'), $data); //crmv@56443
}
...
function getRelationTables($module,$secmodule){
	...
	if($adb->num_rows($ui10_query)>0){
		...
		} elseif(method_exists($secondary_obj,setRelationTables)){
...
function getModuleSequenceField($module) {
	...
	if (!empty($module)) {
		...
		if($cachedModuleFields === false) {
			...
			$seqColRes = $adb->pquery("SELECT fieldname, fieldlabel, columnname FROM ".$table_prefix."_field WHERE uitype=? AND tabid=? and ".$table_prefix."_field.presence in (0,2)", array('4', getTabid($module)));
