...
class CRMVUtils extends SDKExtendableUniqueClass {
	...
	function replaceStringTemplate($template, $params = array(), $clearExtraTags = false) {
		...
		if (!empty($params) && is_array($params)) {
			$search = array_map(create_function('$item', 'return "{".$item."}";'), array_keys($params));
	 ...
	 */
	static function getEasterDate($Year) {
		$G = $Year % 19;
		$C = (int)($Year / 100);
	 ...
	 */
	static function getDynamicHolidaysForYear($year, $country = 'it') {
		$easter = self::getEasterDate($year);
		$holidays = Array(
			date("d-m",$easter), //pasqua ////crmv@33465
			date("d-m",strtotime("+ 1 day",$easter)), //pasquetta ////crmv@33465
		);
		return $holidays;
	}
	 ...
	 */
	static function getFixedHolidaysForYear($year, $country = 'it') {
		$holidays = Array(
			'01-01', //capodanno
			'06-01', //epifania
			'25-04', //liberazione
...
function getDuplicateQuery($module,$field_values,$ui_type_arr)
{
	...
	else
	{
		...
		if($module == 'Contacts' || $module == 'Accounts' || $module == 'Leads'){  // crmv@77469
			...
			Vtiger_Utils::CreateTable($tmptable_orig,implode(",",$field_create),true,true);	//crmv@58258@
			if (VteSession::get('duplicateshandling_empty_flag') !== true){
...
function get_allids($list_query_count,$ids_to_jump = false){
	...
	if ($ids_to_jump){
		...
		$query.=" and $parameter not in (".implode(",",$ids_to_jump).")";
		//mycrmv@160842e
	}
	$res = $adb->query($query);
	//crmv@27096
...
//crmv@19370e
		...
		foreach ($dropped_fields as $field_dropped){
			...
			if ($key!== false && is_array($instance->list_fields[$key])){
				...
				if (is_array($arr) && in_array(key($instance->list_fields[$key]),array_keys($arr))){
					$columns[] = key($instance->list_fields[$key]).".".$field_dropped;
				}
	...
	//crmv@18124
	if ($module == 'Calendar'){
		//crmv@33982	crmv@54924
		if (in_array($table_prefix.'_seactivityrel.crmid',$columns)){
			$columns[array_search($table_prefix.'_seactivityrel.crmid',$columns)] = $table_prefix.'_seactivityrel.crmid AS "activity_related_to"';
		}
		//crmv@33982e	crmv@54924e
...
function getMenuModuleList($otherAll=false) {	/* crmv@30356	crmv@32217	crmv@31250	crmv@42707	crmv@47905bis */
	...
	foreach($module_list as $id => $info) {
		...
		if (!in_array($info['name'], array('Home', 'Messages', 'PDFMaker', 'Rss', 'Dashboard', 'Sms', 'Fax', 'RecycleBin', 'Charts', 'Portal'))) {
	...
	usort($module_list_other, create_function('$a, $b','return ($a[\'translabel\'] > $b[\'translabel\']);'));
...
function isModuleInstalled($module) {	/* crmv@22700	crmv@27624	crmv@25671	crmv@47905bis */
	...
	if (empty($adb) || !Vtiger_Utils::CheckTable($table_prefix.'_tab')) return false;
...
function fix_query_advanced_filters($module,&$query,$columns_to_check=false,$scope=''){ // crmv@129114
	...
	if (is_array($columns_to_check)) {
		foreach ($columns_to_check as $_to_split){
			...
			//crmv@72942
			$sql = "SELECT {$table_prefix}_entityname.tablename, {$table_prefix}_entityname.fieldname
		...
		if (in_array('Calendar', $relmodules)) {
			$query .= "
				union
					...
					where {$table_prefix}_crmentity.setype not in ('Emails', 'Sms', 'Fax')";
			if (!empty($crmid)) {
...
function requestFromPortal() {
	return (preg_match('/vtigerservice.php$/', $_SERVER['SCRIPT_NAME']) > 0);
}
// crmv@142358e
// crmv@91082
...
function checkMailScannerInfoRule($row) {
	...
	if ($row->linktype == 'DETAILVIEWBASIC' && $row->linklabel == 'LBL_DO_NOT_IMPORT_ANYMORE') {
		...
		if (is_numeric($id)) {
			...
			if (!empty($focus->column_fields['mailscanner_action']) && isPermitted($currentModule,'EditView',$id)) {
				return true;
			}
		}
...
function getModuleImg($module, $style='') {
	return '<i class="vteicon icon-module icon-'.strtolower($module).'" data-first-letter="'.strtoupper(substr($module,0,1)).'"></i>';
}
//crmv@104566e
?>
