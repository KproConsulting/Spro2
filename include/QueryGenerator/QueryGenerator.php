...
class QueryGenerator extends SDKExtendableClass { //crmv@42024
	...
	public function getWSField($name, $usePermissions = true) {
		...
		if (empty($field) && !$usePermissions) {
			// check in local cache for other fields
			if (!isset($this->otherFieldsCache[$name])) {
				$res = $adb->pquery('SELECT * FROM '.$table_prefix.'_field WHERE tabid = ? AND fieldname = ?',array(getTabid($this->module),$name));
				$field = WebserviceField::fromQueryResult($adb,$res,0);
				$this->otherFieldsCache[$name] = $field;
			} else {
				$field = $this->otherFieldsCache[$name];
			}
		}
				...
				//crmv@103450e crmv@126096e
					...
					if (in_array('DocumentFolders', $moduleList) && $fieldName == 'folderid') {
						...
						if (count($activeModules) > 0) {
							...
							$alias = 'entityname_fld_'.$field->getFieldId();
							$fieldSql .= "$fieldGlue ($alias.displayname ".$valueSql." AND $alias.setype IN (".$listMods."))";
						}
					}
					// crmv@144125e
				...
				//crmv@129272e
					...
					//crmv@54149e
						...
						if ($casttype !==false){
							...
							if (strpos($valueSql, ':') !== false && ($fieldName == 'date_start' || $fieldName == 'due_date')) {
