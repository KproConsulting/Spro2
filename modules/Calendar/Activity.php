...
class Activity extends CRMEntity {
	...
	function __construct() {
		...
		$this->tab_name = Array($table_prefix.'_crmentity',$table_prefix.'_activity',$table_prefix.'_activitycf');
		$this->tab_name_index = Array($table_prefix.'_crmentity'=>'crmid',$table_prefix.'_activity'=>'activityid',$table_prefix.'_seactivityrel'=>'activityid',$table_prefix.'_cntactivityrel'=>'activityid',$table_prefix.'_salesmanactivityrel'=>'activityid',$table_prefix.'_activity_reminder'=>'activity_id',$table_prefix.'_recurringevents'=>'activityid',$table_prefix.'_activitycf'=>'activityid');
	...
	function save_module($module)
	{
		...
		//mycrmv@166676
		if($this->column_fields['activitytype'] != 'Task'){
			$ENU = EntityNameUtils::getInstance();
			$ENU->saveCachedName('Events', $this->id);
		}
		//mycrmv@166676e
		/* kpro@bid200620181800 migrazione vte18.05 end */

	}
	/** Function to insert values in vtiger_act_reminder_popup table for the specified module
...
$table_prefix.'_user2role on '.$table_prefix.'_user2role.userid='.$table_prefix.'_users.id where '.$table_prefix.'_activity.activityid='.$id;
...
function get_tasksforol($username)
{
  ...
  for($i=0;$i < $adb->num_rows($result1);$i++)
  {
	...
	$query = "select ".$table_prefix."_activity.activityid as taskid, ".implode(',',$column_table_lists)." from ".$table_prefix."_activity
			 ...
			 where ".$table_prefix."_users.user_name='".$username."' and ".$table_prefix."_crmentity.deleted=0 and ".$table_prefix."_activity.activitytype='Task'";
...
function get_calendarsforol($user_name)
{
  ...
  for($i=0;$i < $adb->num_rows($result1);$i++)
  {
	...
	protected function setupTemporaryTable_tmp($tableName, $tabId, $user, $parentRole, $userGroups) { // crmv@81760
		...
		if ($db->isMysql()){
			...
			if (!$db->table_exist($tableName,true)){
				Vtiger_Utils::CreateTable($tableName,"id I(11) NOTNULL PRIMARY,shared I(1) default 0",true,true);
			}
		...
		//crmv@17001
		$res = $db->query("SELECT id FROM ".$table_prefix."_users WHERE id NOT IN (SELECT id FROM $tableName)");
	...
	function save($module_name,$longdesc=false,$offline_update=false,$triggerEvent=true) {
		...
		if (($this->mode == '' || $this->mode == 'create') && empty($this->column_fields['ical_uuid'])) {
	...
	function getActivityDetails($description,$user_id,$from='',$save_mode='',$record='')	//crmv@17001	//crmv@19555
	{
		...
		if($from == "invite" || $from == "invite_con") {
			...
			//$confirm_url = $site_URL."/index.php?module=Calendar&action=ConfirmInvitation&record=$record";
			$confirm_url = $site_URL."/modules/Calendar/ConfirmInvitation.php?from=$from&app_key=$application_unique_key&record=$record&userid=".$user_id;//$current_user->id;
	...
	function getSharedUserId($id){
		...
		$sharedid = Array();
		$query = "SELECT ".$table_prefix."_sharedcalendar.* from ".$table_prefix."_sharedcalendar left join ".$table_prefix."_users on ".$table_prefix."_sharedcalendar.sharedid=".$table_prefix."_users.id where userid=?";
