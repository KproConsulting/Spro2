...
if (!$processMakerView) {
	if(isset($_REQUEST['record']) && $_REQUEST['record'] != '') {
		if(isset($_REQUEST['convertmode']) &&  $_REQUEST['convertmode'] == 'quotetoinvoice') {
			...
			//$associated_prod = getAssociatedProducts("Quotes",$quote_focus);	//crmv@30721
			...
			//$smarty->assign("ASSOCIATEDPRODUCTS", $associated_prod);	//crmv@30721
		...
		elseif(isset($_REQUEST['convertmode']) &&  $_REQUEST['convertmode'] == 'ttickettoiv') {
			...
			//Added to display the TroubleTicket's associated vtiger_products -- when we create SO from Trouble Tickets DetailView
			//$associated_prod = getAssociatedProducts("HelpDesk",$tticket_focus);	//crmv@30721
			$final_details = $InventoryUtils->getFinalDetails("HelpDesk",$tticket_focus);	//crmv@30721
			// $smarty->assign("TTICKETID_ID", $tticketid); para asociar ambas entidades PDTE
	...
	//        $smarty->assign("AVAILABLE_PRODUCTS", 'true');
		...
		elseif(isset($_REQUEST['convertmode']) &&  $_REQUEST['convertmode'] == 'sotoinvoice') {
			...
			/* kpro@tom150620181140 migrazione vte18.05 */
			$focus->column_fields['kp_business_unit'] = $so_focus->column_fields['kp_business_unit'];	
			$focus->column_fields['commessa'] = $so_focus->column_fields['commessa'];
			/* kpro@tom150620181140 migrazione vte18.05 */

			//Added to display the SalesOrder's associated vtiger_products -- when we create vtiger_invoice from SO DetailView
			//$associated_prod = getAssociatedProducts("SalesOrder",$so_focus);	//crmv@30721
		...
		elseif(isset($_REQUEST['convertmode']) &&  $_REQUEST['convertmode'] == 'ddttoinvoice') {
			...
			//Added to display the SalesOrder's associated vtiger_products -- when we create vtiger_invoice from SO DetailView
			//$associated_prod = getAssociatedProducts("Ddt",$ddt_focus);	//crmv@30721
...
if (!$processMakerView) {
	if(isset($_REQUEST['opportunity_id']) && $_REQUEST['opportunity_id'] !='') {
		...
		//$associated_prod = getAssociatedProducts("Potentials",$potfocus,$potfocus->column_fields['potential_id']);	//crmv@30721
...
elseif($focus->mode == 'edit') {
	...
	//$associated_prod = getAssociatedProducts("Invoice",$focus);	//crmv@30721
	//$smarty->assign("ASSOCIATEDPRODUCTS", $associated_prod);
	$smarty->assign("MODE", $focus->mode);
	$final_details = $InventoryUtils->getFinalDetails('Invoice', $focus);	//crmv@30721
}
elseif(isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true') {
	//$associated_prod = $INVOICE_final_details;
	//$smarty->assign("AVAILABLE_PRODUCTS", 'true');
	$smarty->assign("MODE", $focus->mode);
