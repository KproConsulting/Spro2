	...
	public function calcNextExecution($scheduling, $now = null) {
		...
		if($scheduleType == self::SCHEDULED_HOURLY) {
			...
			if($scheduledDayOfMonth == $currentDayOfMonth) {
				...
				if($scheduledDayOfMonth < $currentDayOfMonth) {
					/* kpro@bid051220181600 */
					//mycrmv@172850
					$nextMonth = strtotime('next month');
					$monthInFullText = date('F',$nextMonth);
					$yearFullNumberic = date('Y',$nextMonth);
					//mycrmv@172850
					/* kpro@bid051220181600 end */
				}
			...
			if(($scheduledMonth+1) == $currentMonth) {
				if (strtotime(date("Y-m-d {$scheduledTime}",$now)) < $now){
					return date("Y-m-d H:i:s",strtotime('+1 year '.$scheduledTime,$now));
				}
				else{
					return date("Y-m-d H:i:s",strtotime($scheduledTime,$now));
				}				
