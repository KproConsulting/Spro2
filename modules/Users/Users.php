...
require_once('include/utils/UserAuthtoken.php'); //crmv@29377
...
class Users extends CRMEntity { //crmv@392267
	...
	var $time_to_change_old_pwd = 5;	//(days) days after the password is expired to show the change pwd form
	...
	function doLogin($user_password)
	{
		...
		if ($AUTHCFG[active] && $usr_name != 'admin'){
	...
	function trackSuccessLogin() {
		...
		$result = $adb->pquery("select * from {$this->track_login_table} where userid = ? and ip = ? and type = ? and status = ?",array($userid, $ip, $type, ''));
	...
	function getLoginType() {
		...
		if (empty($type)) {
			...
			} elseif (basename(getenv('SCRIPT_FILENAME')) == 'webservice.php') {
				$type = 'ws';	// webservices
			} else {
				$type = 'web';
			}
	...
	function sendMailLockedLogin($id, $ip, $type = 'web') {
		...
		if ($type == 'as') {
			...
			$body = sprintf($body, $this->as_login_time_to_lock, $ip, '<a href="'.$site_URL.'/modules/Users/SetLoginWhiteList.php?k='.$mailkey.'">'.$here.'</a>');
		} else {
			$subject = getTranslatedString('LBL_MAIL_LOCKED_LOGIN_SUBJECT','Users');
			...
			$body = sprintf($body, $max_num, $ip, '<a href="'.$site_URL.'/modules/Users/SetLoginWhiteList.php?k='.$mailkey.'">'.$here.'</a>');
		}
...
//		$this->loadPreferencesFromDB($row['user_preferences']);
				...
				*/
				...
				if (!empty($free_login)) {
					if ($free_login == 'CONNECTION_FAILED' && $this->time_to_block_user != 0 && !in_array($last_login,$empty_date_values) && ($now > strtotime($last_login.' + '.$this->time_to_block_user.' months'))) {
						...
						if (!$this->checkPasswordCriteria($user_password,$row)) {
							VteSession::set('login_confirm', array('text'=>sprintf(getTranslatedString('LBL_NOT_SAFETY_PASSWORD','Users'),$this->password_length_min).' '.getTranslatedString('LBL_CLICK_OK_TO_RECOVER','Users'),'action'=>"window.location = 'modules/Users/Recover.php';"));
						}
				...
				//crmv@35153e
					...
					if ($this->time_to_block_user != 0 && !in_array($last_login,$empty_date_values) && ($now > strtotime($last_login.' + '.$this->time_to_block_user.' months'))) {
						...
						$check_pwd_error = true;
						// crmv@43592
						if ($this->time_to_change_old_pwd > 0 && ($now < strtotime($last_change_pwd.' + '.$this->time_to_change_password.' months '.$this->time_to_change_old_pwd.' days'))) {
	...
	//crmv@392267e
		...
		$KpMHW = KpSDK::registraModuleHomeStandard(null, null, $this->id, "Users");
		/* kpro@bid16062017 end */
		/* kpro@tom150620181140 migrazione vte18.05 end */
		
		//crmv@161021
		$focus = CRMEntity::getInstance('Employees');
		if ($synchronizeEmployee && $focus->synchronizeUser) $focus->syncUserEmployee($this->id,$this->mode);
		//crmv@161021e
	}
	//crmv@392267e
	/**
	...
	*/
	...
	function transformOwnerShipAndDelete($userId,$transformToUserId){
		...
		$sql = "delete from ".$table_prefix."_users where id=?";
	...
	//crmv@104988 crmv@115336 crmv@152712
	...
	function formatUserNameSql($adb, $table, $with_name=false, $username_format=null) {
		if ($with_name) {
			if (empty($username_format)) $username_format = $this->username_format;
			if ($username_format == self::USERNAME_FORMAT_STANDARD) {
				return $adb->sql_concat(Array($table.".user_name","' ('",$table.".first_name","' '",$table.".last_name","')'"));
			} elseif ($username_format == self::USERNAME_FORMAT_INVERTED) {
				return $adb->sql_concat(Array($table.".first_name","' '",$table.".last_name","' ('",$table.".user_name","')'"));
			}
		}
		return $table.".user_name";
	}
	//crmv@126096e crmv@115336e crmv@152712e
