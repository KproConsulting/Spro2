...
//crmv@33801e
{
	...
	public $showProcessGraphTab;	//crmv@101506
	...
	if($this->mode == 'edit')
	{
		...
		else
		{
			...
			$perm_qry = "SELECT columnname FROM ".$table_prefix."_field INNER JOIN ".$table_prefix."_def_org_field ON ".$table_prefix."_def_org_field.fieldid = ".$table_prefix."_field.fieldid WHERE ".$table_prefix."_field.tabid = ? AND ".$table_prefix."_def_org_field.visible = 0 and ".$table_prefix."_field.tablename='".$table_prefix."_crmentity' and ".$table_prefix."_field.displaytype in (1,3) and ".$table_prefix."_field.presence in (0,2) ";
 			//crmv@60969
 			$perm_qry.=" AND EXISTS(SELECT * FROM ".$table_prefix."_profile2field 
		...
		if(isset($this->column_fields[$fieldname]))
		{
			//crmv@130458
		  	...
		  	//crmv@130458e
				...
				elseif($uitype == 5 || $uitype == 6 || $uitype ==23)
				{
					...
					else
					{
						...
						if ($module == 'Calendar' && $fieldname == 'date_start') {
							...
							$newval = adjustTimezone($newval, 0, null, true);
							...
							$fldvalue = adjustTimezone($fldvalue, 0, null, true);
						}
				...
				elseif($uitype == 70)
				{
					$fldvalue = adjustTimezone($this->column_fields[$fieldname], 0, null, true);
				}
				// crmv@25610e
				elseif($uitype == 7)
				{
  ...
  function deleteRelation($table_name)
  {
	...
	function track_view($user_id, $current_module, $id='')
	{
		...
		//crmv@35105
		$modnotification = CRMEntity::getInstance('ModNotifications');
	...
	*/
	...
	function get_related_buttons($this_module, $this_crmid, $for_module, $actions, $depField = '') {
		...
		if ($for_module == 'Calendar') {
		    //crmv@62447
			if (in_array('ADD', $actions) && isPermitted($for_module,1, '') == 'yes' && $fieldPerm) {
			...
			if (in_array('SELECT', $actions) && isPermitted($for_module,4, '') == 'yes' && empty($depField)) {
			...
			if (in_array('ADD', $actions) && isPermitted($for_module,1, '') == 'yes' && $fieldPerm) {
				if (PerformancePrefs::getBoolean('ADD_RELATION_IN_FULL_PAGE')) { // crmv@115378
					$onclick = "onclick='this.form.action.value=\"EditView\";this.form.module.value=\"$for_module\"' type='submit' name='button'";
				} else {
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions=false, $direction = null) { // crmv@125816
		...
		global $currentModule, $app_strings, $singlepane_view, $table_prefix, $current_user;
		...
		if($related_module != 'Contacts'){ /* kpro@bid200620181800 migrazione vte18.05 */
			// crmv@161254 crmv@161515
			if (is_array($other->tab_name)) {
				foreach($other->tab_name as $tab_name){
					if(strpos($query,$tab_name) === false){
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($numOfFields > 0 || $inventoryModule) {	
			...
			$query .= " WHERE ".$table_prefix."_crmentity.deleted = 0 $setypeCond AND $aliasTable.$this->table_index = $id";	//crmv@102767
			// crmv@64325e
			if ($related_module == 'ChangeLog') $query .= " AND {$other->table_name}.hide = 0";	//crmv@135193
		...
		foreach($with_crmid as $relcrmid) {
			...
			if (!empty($relatedInstance)) {
				...
				if ($checkpresence && $adb->num_rows($checkpresence) == 0) {
					...
					$adb->pquery("UPDATE {$table_prefix}_crmentity SET modifiedtime = ? WHERE crmid IN (?,?)", array($adb->formatDate(date('Y-m-d H:i:s'), true), intval($thisId), intval($relatedId))); // crmv@49398	crmv@56655 crmv@69690
				}
			// use standard crmentityrel
				...
				if ($checkpresence && $adb->num_rows($checkpresence) == 0) {
					...
					$adb->pquery("UPDATE {$table_prefix}_crmentity SET modifiedtime = ? WHERE crmid IN (?,?)", array($adb->formatDate(date('Y-m-d H:i:s'), true), intval($crmid), intval($relcrmid))); // crmv@49398	crmv@56655 crmv@69690
				}
			}
		}
	}
...
//			}
	...
	public function getFixedOrderBy($module,$order_by,$sorder){
		...
		//crmv@127820e
			...
			if ($tablename == '' && $order_by == 'crmid') $tablename = $table_prefix.'_crmentity';
	...
	function getListQuery($module, $where='') {
		...
		// crmv@139482
		$linkedModulesQuery = $this->db->pquery("SELECT distinct fieldname, columnname, tablename, relmodule FROM ".$table_prefix."_field" .
				" INNER JOIN ".$table_prefix."_fieldmodulerel ON ".$table_prefix."_fieldmodulerel.fieldid = ".$table_prefix."_field.fieldid" .
		...
		for($i=0; $i<$linkedFieldsCount; $i++) {
			...
			if (in_array($other->table_name,$usedTables)) {
				$query .= " LEFT JOIN $other->table_name AS {$other->table_name}2 ON {$other->table_name}2.$other->table_index = $tablename.$columnname";
			} else {
		...
		if ($viewId != '') {
			...
			$reportFilter = $oCustomView->getReportFilter($viewId);
			if ($reportFilter) {
				$query .= $oCustomView->getReportFilterJoin("{$table_prefix}_crmentity.crmid", $reportFilter,$current_user->id); // crmv@122906
			}
		}
		//crmv@31775e
		// crmv@30014
	...
	function getMessagePopupLinkQuery(&$queryGenerator, $module, $relatedIds = array(), $searchstr = '', $excludeIds = array()) {
		...
		if (PerformancePrefs::getBoolean('USE_TEMP_TABLES', true)) {
			...
			if (!$adb->table_exist($tableName,true)){
				//crmv@64361
				if ($adb->isMssql()){
					Vtiger_Utils::CreateTable($tableName,"id I(11) NOTNULL, sequence I(11) AUTOINCREMENT NOTNULL PRIMARY",true,true);
				} else {
					Vtiger_Utils::CreateTable($tableName,"id I(11) NOTNULL UNIQUE INDEX, sequence I(11) AUTOINCREMENT NOTNULL PRIMARY",true,true);
				}
				//crmv@64361 e
			}
...
//		$queryGenerator->appendToWhereClause(" AND {$table_prefix}_messagesrel.messagehash IS NULL");
	...
	function getMessagePopupOrderBy(&$queryGenerator, $module, $relatedIds = array(), $searchstr = '') {
		global $table_prefix;
		return " ORDER BY {$table_prefix}_crmentity.modifiedtime DESC";
	}

	function get_messages_list($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		global $log, $singlepane_view,$currentModule,$current_user,$adb,$table_prefix;
		...
		foreach ($listfields as $label=>$finfo) {
			if ($finfo[0] != $table_prefix.'_crmentity') {
		...
		if (PerformancePrefs::getBoolean('USE_TEMP_TABLES')) {
			$tmpSelects = "MAX({$table_prefix}_crmentity.crmid) AS id";
		} else {
			$id = intval($id);
			$tmpSelects = "{$current_user->id} as userid, $id as parentid, MAX({$table_prefix}_crmentity.crmid) AS id";
		}
		// crmv@113604
		$query = "SELECT $tmpSelects
		...
		if($defOrgSharingPermission[$rel_tab_id] != 3) {
			$query .= " LEFT JOIN {$table_prefix}_messages_sfolders 
							ON {$table_prefix}_messages_sfolders.userid = {$table_prefix}_crmentity.smownerid 
		...
		$query .= " WHERE {$table_prefix}_crmentity.deleted = 0 $setypeCond";
		// crmv@63349e crmv@64325e
		...
		if($defOrgSharingPermission[$rel_tab_id] == 3 || $defOrgSharingPermission[$rel_tab_id] == 8) {
			...
			if (PerformancePrefs::getBoolean('USE_TEMP_TABLES', true)) {
				...
				$query .= " OR {$table_prefix}_crmentity.smownerid IN (select id from $tableName)";
			} else {
				$tableName = $table_prefix.'_tmp_users';
				$sharedTabId = null;
				if(!empty($sharingRuleInfo) && (count($sharingRuleInfo['ROLE']) > 0 ||
				...
				$query .= " OR {$table_prefix}_crmentity.smownerid = {$current_user->id}";
				//crmv@160797
				if ($defOrgSharingPermission[$rel_tab_id] == 3) {
					if ($sharedTabId > 0) {
						$query .= " OR {$table_prefix}_crmentity.smownerid IN (select subuserid from $tableName WHERE userid = {$current_user->id} AND tabid = $sharedTabId)";
					} else {
						$query .= " OR {$table_prefix}_crmentity.smownerid IN (select subuserid from $tableName WHERE userid = {$current_user->id})";
					}
	...
	function getEntityPreview($id,$module='') {
		...
		$img_check = resourcever($img_check); // crmv@140887
	...
	function setupTemporaryRelTable_tmp($module,$secmodule,$id='',$relationinfo=Array()){ // crmv@63349
		...
		elseif($db->isMssql()){
			if (!$db->table_exist($tableName,true)){
				Vtiger_Utils::CreateTable($tableName,"crmid I(19) key,relcrmid I(19) key",true,true);
			} else {
	...
	function getExtraDetailBlock() {
		...
		if (vtlib_isModuleActive('Processes')) $extraBlock .= $this->generateProcessGraph();	//crmv@149529
	...
	//crmv@149529
	function generateProcessGraph() {
		global $adb, $table_prefix, $currentModule;
		$return = '';
		
		$this->showProcessGraphTab = false;
		...
		if (in_array($currentModule, $relatedToProcesses)) {
			$rm = RelationManager::getInstance();
			$ids = $rm->getRelatedIds($currentModule, $this->id, 'Processes');	//TODO order by createdtime desc
			if (!empty($ids)) {
				$this->showProcessGraphTab = true;
				
				require_once('modules/Settings/ProcessMaker/ProcessMakerUtils.php');
				$PMUtils = ProcessMakerUtils::getInstance();
				$relatedTo = $PMUtils->getProcessRelatedTo($this->id,'processesid');	//crmv@93990
				...
				$return = $focus->getExtraDetailBlock($selectionProcesses);
			}
		}
		return $return;
	}
