...
if(isset($_GET['data_manutenzione'])){
    $data_manutenzione = htmlspecialchars(addslashes(html_entity_decode(strip_tags($_GET['data_manutenzione']), ENT_QUOTES,$default_charset)), ENT_QUOTES,$default_charset);
    $data_manutenzione = substr($data_manutenzione,0,100);
...
if($data_manutenzione != ""){
    $q_manutenzioni .= " AND t.data_manutenzione = '".$data_manutenzione."'";
}
...
for($i=0; $i<$num_manutenzioni; $i++){
    ...
    $numero_rilevazioni_in_corso = verificaSeRilevazioneInCorso($manutenzioniid);
    ...
    if($stato_manutenzione != "Creata" || ($stato_manutenzione == "Creata" && $tipo_manutenzione == "Programmata" && $resp_man_ord == "si") || ($stato_manutenzione == "Creata" && $tipo_manutenzione == "Extra" && $resp_man_extra == "si")){
		...
		if($percentuale_fermo == "default" || $percentuale_fermo == $percentuale_fermo_impianto){
	        ...
	        $rows[] = array('id' => $manutenzioniid,
                            ...
                            'numero_rilevazioni_in_corso' => $numero_rilevazioni_in_corso);
...
function verificaSeRilevazioneInCorso($manutenzione){
    global $adb, $table_prefix, $current_user, $default_charset;

    $q_lista_tempi = "SELECT 
                        COALESCE(COUNT(*), 0) numero_rilevazioni_in_corso
                        FROM {$table_prefix}_tempimanutenzioni tempi
                        INNER JOIN {$table_prefix}_crmentity ent ON ent.crmid = tempi.tempimanutenzioniid
                        INNER JOIN {$table_prefix}_tecnicimanutentori tec ON tec.tecnicimanutentoriid = tempi.kp_risorsa
                        INNER JOIN {$table_prefix}_manutenzioni man ON man.manutenzioniid = tempi.kp_manutenzione
                        INNER JOIN {$table_prefix}_crmentity entman ON entman.crmid = man.manutenzioniid
                        WHERE ent.deleted = 0 AND (tempi.kp_data_fine_reg IS NULL OR tempi.kp_data_fine_reg = '' OR tempi.kp_data_fine_reg = '0000-00-00') 
                        AND tempi.kp_manutenzione = ".$manutenzione;
    $res_lista_tempi = $adb->query($q_lista_tempi);
    
    if($adb->num_rows($res_lista_tempi) > 0){
        
        $numero_rilevazioni_in_corso = $adb->query_result($res_lista_tempi, 0, 'numero_rilevazioni_in_corso');
        $numero_rilevazioni_in_corso = html_entity_decode(strip_tags($numero_rilevazioni_in_corso), ENT_QUOTES, $default_charset);

    }
    else{

        $numero_rilevazioni_in_corso = 0;

    }

    return $numero_rilevazioni_in_corso;
