...
function getTabid2($module) {
	...
	$key = "module_tabid_".$module;
	
	$tabid = $cache->get($key);
	if ($tabid !== false && $tabid !== null) return $tabid;
...
function getBlocks($module,$disp_view,$mode,$col_fields='',$info_type='',&$blockVisibility='')	//crmv@99316
{
	...
	$query = 
		...
		WHERE {$vh_info['blocks'][1]} and $disp_view = 0 and visible = 0
		ORDER BY sequence";
	$result = $adb->pquery($query, array($vh_info['blocks'][2]));
	...
	$col = 'mandatory';
  	...
  	{
  		//crmv@7216e
  		$sql = "SELECT {$vh_info['field'][0]}.fieldid, MIN({$vh_info['profile2field'][0]}.$col) AS \"mandatory\"
	...
	{
  		//crmv@56398
  		$sql = "SELECT {$vh_info['field'][0]}.*, CASE WHEN (q.$col is not null) THEN q.$col ELSE 1 END AS \"mandatory\" 
			FROM ($sql) q 
...
function getHeaderArray()
{
    ...
    {	
		 ...
		 * Questa modifica serve a nascondere le aree non attive
		 */

		if( !kpTabAttivo($parid) ){
			continue;
		}
...
function kpTabAttivo($id){
	global $adb, $table_prefix;

	$query = "SELECT 
				parenttabid,
				parenttab_label 
				FROM ".$table_prefix."_parenttab 
				WHERE hidden=0 AND parenttabid = ".$id;
...
function create_parenttab_data_file() {
	...
	$result = $adb->query("select parenttabid,parenttab_label from ".$table_prefix."_parenttab where visible=0 and hidden=0 order by sequence");
...
function Button_Check($module)
{
	...
	if ($_REQUEST['action'] == 'index' || $_REQUEST['action'] == 'HomeView' || $_REQUEST['action'] == 'ListView') { // crmv@160778
