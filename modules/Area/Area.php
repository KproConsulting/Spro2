...
class Area extends SDKExtendableClass {
	...
	function searchModule($module,$search,$orberby='',$header='',$hide_other_button=false) {
		...
		$list_result = $adb->limitQuery($list_query,0,$list_max_entries+1);
...
class AreaManager extends SDKExtendableUniqueClass {
	...
	var $hide_fixed_modules = array('Charts','PDFMaker','Reports','Rss','Portal','Dashboard','RecycleBin');
	//crmv@107077e
	...
	function __construct($searchByUser=true) {
		...
		if ($this->searchByUser && Vtiger_Utils::CheckTable($this->table_menu)) {
	...
	function getSelectableModuleList($area,$modules2exclude) {
		...
		usort($list, create_function('$a, $b','return ($a[\'translabel\'] > $b[\'translabel\']);'));
		return $list;
	}
	...
	function getModuleList($area=false,$module=false) {
		...
		if ($_REQUEST['module'] == 'Update') {
			if (!Vtiger_Utils::CheckTable($this->table) && !Vtiger_Utils::CheckTable($this->table_menu)) {
				return false;
			}
		}
		...
		if ($this->searchByUser) {
			 ...
			 *
			 * Questa modifica permette a tutti gli utenti di avere la configurazione delle aree di admin
			 */
		...
		while($info=$adb->fetchByAssoc($res)) {
			...
			// crmv@140887
			$info['img'] = resourcever("themes/images/modulesimg/{$info['name']}.png");
			if (!file_exists($info['img'])) {
				$info['img'] = resourcever("themes/images/modulesimg/Generic.png");
			}
			// crmv@140887e
		...
		if ($areaid == -1) {
			...
			if (!empty($this->hide_fixed_modules)) {
				...
				foreach ($this->hide_fixed_modules as $m) {
					$inst = Vtiger_Module::getInstance($m);
					$tmp_tabids[] = $inst->id;
				}
				if (!empty($tabids)) {
	...
	function getToolValue($param) {
		global $adb;
		if (Vtiger_Utils::CheckTable($this->table_tools)) {
