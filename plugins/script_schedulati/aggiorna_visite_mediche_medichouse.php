...
function AggiornaVisiteMediche($azienda,$giorni_in_scadenza){
	...
	for($i=0; $i<$num_mansioni_risorsa; $i++){
		...
		for($y=0; $y<$num_tipivisita; $y++){
			...
			$q_visita = "SELECT evm.visita_medica,
						evm.data_visita,
						evm.data_fine_validita,
						evm.description
			...
			if($adb->num_rows($res_visita)>0){
				...
				$tipo_visita_specifico = $adb->query_result($res_visita,0,'description');
				$tipo_visita_specifico = html_entity_decode(strip_tags($tipo_visita_specifico), ENT_QUOTES,$default_charset);
				if($tipo_visita_specifico == null){
					$tipo_visita_specifico = '';
				}
			...
			$q_verifica = "SELECT situazvisitemedid FROM {$table_prefix}_situazvisitemed
							INNER JOIN {$table_prefix}_crmentity ON crmid = situazvisitemedid
							WHERE deleted = 0 AND tipo_visita = ".$tipivisitamedid." AND mansione_risorsa =".$mansionirisorsaid;
			...
			if($adb->num_rows($res_verifica)>0){
				...
				$q_upd_sit = "UPDATE {$table_prefix}_situazvisitemed SET
								...
								kp_tipo_visita_spec = '".$tipo_visita_specifico."',
								aggiornato = '1'
				...
				$situazione_visite->column_fields['kp_tipo_visita_spec'] = $tipo_visita_specifico;
	...
	$q_risorse = "SELECT contactid FROM {$table_prefix}_contactdetails
					INNER JOIN {$table_prefix}_crmentity ON crmid = contactid
					WHERE deleted = 0 AND accountid = ".$azienda;						
	...
	for($i=0; $i<$num_risorse; $i++){
		...
		$q_sit_vis = "SELECT situazvisitemedid, stato_sit_visita FROM {$table_prefix}_situazvisitemed
						INNER JOIN {$table_prefix}_crmentity ON crmid = situazvisitemedid
						WHERE deleted = 0 AND risorsa = ".$contactid;
