...
{elseif $uitype eq '10'}
		...
		<div>
			<span class="dvtCellLabel">
				...
				{if $FIELDHELPINFO && $FIELDHELPINFO.$fldname}
					...
					<i class="vteicon md-sm valign-bottom" onclick="vtlib_field_help_show(this, '{$fldname}');">help</i> {* kpro@bid230520181800 *}
	...
	<table cellspacing="0" cellspacing="0" width="100%">
		...
		{else}
			<td width="20%" class="{$DIVCLASS}" style="padding:0px 5px;vertical-align:middle;text-align:center;">
				{if $fromlink eq 'qcreate'}
					<select id="{$fldname}_type" class="detailedViewTextBox" name="{$fldname}_type" onChange='reloadAutocomplete("{$fldname}","{$fldname}_display","module="+this.value+"{$popup_params}"); document.QcEditView.{$fldname}_display.value=""; document.QcEditView.{$fldname}.value=""; enableReferenceField(document.QcEditView.{$fldname}_display);'>	{* crmv@29190 *}
				{else}
					<select id="{$fldname}_type" class="detailedViewTextBox" name="{$fldname}_type" onChange='reloadAutocomplete("{$fldname}","{$fldname}_display","module="+this.value+"{$popup_params}"); document.EditView.{$fldname}_display.value=""; document.EditView.{$fldname}.value=""; if (typeof($("qcform")) != "undefined" && $("qcform") != null) $("qcform").innerHTML=""; enableReferenceField(document.EditView.{$fldname}_display);'>	{* crmv@29190 crmv@48241 *}
				{/if}
		...
		<div {if $fld_displayvalue|trim eq ''}class="{$DIVCLASS}"{else}class="dvtCellInfoOff"{/if} {if $smarty.request.enable_editoptions eq 'yes' && $FIELDTYPE eq 'Other'}style="display:none"{/if} style="position:relative">	{* crmv@92272 *}
			...
			<div class="dvtCellInfoImgRx">
				...
				<i class="vteicon md-link" tabindex="{$vt_tab}" title="{$APP.LBL_SELECT}" onclick='openPopup("index.php?module="+document.{$editViewType}.{$fldname}_type.value+"&action=Popup&html=Popup_picker&form=vtlibPopupView&forfield={$fldname}&srcmodule={$MODULE}&forrecord={$ID}{$extra_popup_params}{$SDK->getPopupHiddenElements($MODULE,$fldname)}","test","width=640,height=602,resizable=0,scrollbars=0,top=150,left=200");'>view_list</i> {* crmv@126696 *}
				{* crmv@37211 *}
		...
		{if $smarty.request.enable_editoptions eq 'yes'}
			<div {if $fld_displayvalue|trim eq ''}class="{$DIVCLASS}"{else}class="dvtCellInfoOff"{/if} id="div_other_{$fldname}" {if $FIELDTYPE neq 'Other'}style="display:none"{/if}">
				<select id="other_{$fldname}" name="other_{$fldname}" class="detailedViewTextBox"></select>
			</div>
		{/if}
		{* crmv@92272e *}
		</td>
...
{elseif $uitype eq 11 || $uitype eq 1 || $uitype eq 13 || $uitype eq 7 || $uitype eq 9 || $uitype eq 1112 || $uitype eq 1013 || $uitype eq 1014}
	...
	{if $fldname eq 'tickersymbol' && $MODULE eq 'Accounts'}
		<div class="{$DIVCLASS}">
			<input type="text" tabindex="{$vt_tab}" name="{$fldname}" id ="{$fldname}" value="{$fldvalue}" class="detailedViewTextBox" onBlur="{if $fldname eq 'tickersymbol' && $MODULE eq 'Accounts'}sensex_info(){/if}" {if $GRIDSEARCH eq true}onkeyup="callGridSearch(this,'input','{$FOLDERID}');"{/if}>
		</div>
	{else}
		<!--   //crmv@7231 -->
...
{elseif $uitype eq 15 || $uitype eq 16 || $uitype eq 1015}	{* <!-- DS-ED VlMe 31.3.2008 - add uitype 504 --> *}
	{include file="FieldHeader.tpl" uitype=$uitype mandatory=$keymandatory label=$fldlabel massedit=$MASS_EDIT}
	<div class="{$DIVCLASS}">
   		<select id="{$fldname}" name="{$fldname}" tabindex="{$vt_tab}" class="detailedViewTextBox" {if $GRIDSEARCH eq true}onChange="callGridSearch(this,'select','{$FOLDERID}');"{/if}>
...
{elseif $uitype eq 33}
	...
	{else}
		<div class="{$DIVCLASS}">
			<select MULTIPLE name="{$fldname}[]" size="4" tabindex="{$vt_tab}" class="detailedViewTextBox">
			{foreach item=arr from=$fldvalue}
				<option value="{$arr[1]}" {$arr[2]}>{$arr[0]}</option>
			{/foreach}
			</select>
...
{elseif $uitype eq 53}
	...
	{else}
		...
		{assign var=style_other value='display:none'}
	{/if}
	{if $assigntype eq 'O' || ($smarty.request.enable_editoptions eq 'yes' && $smarty.request.assigned_user_id neq '' && $smarty.request.assigned_user_id|is_numeric === false)}
		{assign var=select_user value=''}
		{assign var=select_group value=''}
	...
	<table cellspacing="0" cellspacing="0" width="100%">
		<tr>
			...
			{* crmv@57221e *}
			<td width="20%" class="{$DIVCLASS}" style="padding:0px 5px;vertical-align:middle;text-align:center;">
				{if $secondvalue neq '' || $smarty.request.enable_editoptions eq 'yes'}	{* crmv@139856 *}
					{if $fromlink eq 'qcreate'}
			...
			<td width="{if $OLD_STYLE eq true}70{else}80{/if}%" style="position:relative">	{* crmv@57221 *} {* crmv@98866 *}
				...
				{if $smarty.request.enable_editoptions eq 'yes'}
					<div {if $fld_displayvalue|trim eq ''}class="{$DIVCLASS}"{else}class="dvtCellInfoOff"{/if} id="{$assign_other_div}" style="position:relative; {$style_other}">
						...
						<div class="dvtCellInfoImgRx">
							<i style="display:none" id="advanced_field_assignment_button_{$fldname}" class="vteicon md-link" title="{'LBL_PM_ADVANCED_FIELD_ASSIGNMENT'|getTranslatedString:'Settings'}" onClick="ActionTaskScript.openAdvancedFieldAssignment('{$smarty.request.id}','{$smarty.request.elementid}','{$smarty.request.action_id}','{$fldname}','{$MODULE}','popup',true)">more_horiz</i>
						</div>
					</div>
				{/if}
				{* crmv@92272 *}
...
{elseif $uitype eq 56}
	{if $secondvalue eq 'picklist'}
		{include file="EditViewUI.tpl" uitype=15 fldvalue=$thirdvalue}
	{else}
		{include file="FieldHeader.tpl" uitype=$uitype mandatory=$keymandatory label=$fldlabel massedit=$MASS_EDIT}
...
{elseif $uitype eq 23 || $uitype eq 5 || $uitype eq 6}
	...
	<div class="{$DIVCLASS}">
		...
		{* crmv@82419 crmv@100585 *}
			<input name="{$fldname}" class="form-control" tabindex="{$vt_tab}" id="jscal_field_{$fldname}" type="text"  size="11" maxlength="10" value="{$date_val}" {if $fromlink eq 'qcreate' && $fldname eq 'date_start'}onchange="parent.calDuedatetimeQC(this.form,'date');"{/if}  {if $smarty.request.enable_editoptions eq 'yes'}style="display:none"{/if}>	{* //crmv@31315 *}
		</td>
		{* crmv@22583 *}
		...
		<tr>
		<td colspan="2" {if $smarty.request.enable_editoptions eq 'yes'}style="display:none"{/if}>
...
{elseif $uitype eq 55 || $uitype eq 255}
	...
	<div class="{$DIVCLASS} field-horiz">
		...
		{/if}
		<input type="text" id="{$fldname}" name="{$fldname}" tabindex="{$vt_tab}" class="detailedViewTextBox" {if $uitype eq 55}style="width:58%;"{/if} value= "{$secondvalue}" >
	</div>
{elseif $uitype eq 69}
	{include file="FieldHeader.tpl" uitype=$uitype mandatory=$keymandatory label=$fldlabel massedit=$MASS_EDIT}
	<div class="{$DIVCLASS}">
