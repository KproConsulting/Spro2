...
class PDFContent extends SDKExtendableClass { // crmv@42024
    ...
    function  __construct($templateid, $module, $focus, $language) {
      ...
      global $$db, $$mod, $$vcv, $$salt, $PDFMaker_template_id;
    ...
    protected function convertRelatedModule() {
    	global $table_prefix;
      $sql = "SELECT fieldid, fieldname, uitype
      ...
      if ($num_rows > 0)
      {
        while($row = $this->db->fetch_array($result))
        {
          ...
          switch ($row["uitype"]) {
          	/* crmv@126096 old uitypes
            case "51": $related_module = "Accounts"; break;
            case "57": $related_module = "Contacts"; break;
            case "58": $related_module = "Campaigns"; break;
            case "59": $related_module = "Products"; break;
            case "73": $related_module = "Accounts"; break;
            case "75": $related_module = "Vendors"; break;
            case "81": $related_module = "Vendors"; break;
            case "76": $related_module = "Potentials"; break;
            case "78": $related_module = "Quotes"; break;
            case "80": $related_module = "SalesOrder"; break;
            case "68":
            */
            case "10": $related_module = getSalesEntityType($fk_record); break;
          }
    ...
    function convertInventoryModules() {
      global $table_prefix;
	  ...
	  if ($num_rows > 0)
      {
        ...
        if (count($Products["TOTAL"]["VATBLOCK"]) > 0){
            ...
            foreach ($Products["TOTAL"]["VATBLOCK"] as $keyW=>$valueW){
                if($valueW["netto"]!=0 && $valueW["vat"] != 0) {
                    /*$vattable .= "<tr>
                    ...
                    $vattable .= "<tr>
                                  <td colspan='9' align='left'><span style='font-size:12px;'>".$valueW["label"]." (".$this->formatNumberToPDF($valueW["value"])." %)</span></td>
                                  <td align='right'><span style='font-size:12px;'>".$this->formatNumberToPDF($valueW["vat"])."</span></td>
                                </tr>";
                }
			      }
            //$vattable .= "</table>";
    ...
    protected function replaceFieldsToContent($emodule, $efocus, $is_related = false, $is_inventory = false) {
	  ...
	  //crmv@32899
	  $moduleInstance = Vtiger_Module::getInstance($emodule);
	...
	//crmv@19166e crmv@150773e
      ...
      for($i=1; $i<=$num_rows; $i++)
      {
        ...
        /* kpro@bid200620181800 migrazione vte18.05 */
        /* kpro@bid291120171400 */
        $post_tasse = $InventoryUtils->KpDatiProdottoServizio($psid);
        if(!$post_tasse){
      ...
      $totalinfo['id_configurazione'] = $InventoryUtils->KpGetConfigurazioneDaFattura($this->focus->id); /* kpro@bid291120171400 */ /* kpro@bid200620181800 migrazione vte18.05 */
    ...
    protected function retrieve_entity_infoCustom(&$focus,$record,$module) {
      ...
      foreach($focus->tab_name_index as $table_name=>$index)
        $result[$table_name] = $this->db->pquery("SELECT * FROM ".$table_name." WHERE ".$index."=?", array($record));
              ...
              foreach($RelatedBlocks[1] AS $r => $relblockid)
              {
                  if (!in_array($relblockid,$ConvertRelBlock))
                  {
                      ...
                      $secmodule = $this->db->getOne($sql_rb,0,"secmodule");
						...
						//crmv@25443
						global $adb;
						$secmoduleInstance = Vtiger_Module::getInstance($secmodule);
