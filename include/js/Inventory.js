...
function calcGrandTotal() {
    ...
    jQuery('#proTab input[id^=hdnProductId]').each(function(index, item) {
        var pid = item.id.replace('hdnProductId', '');
        if (jQuery('#deleted' + pid).val() == 0) {
            var post_tassa = KpDatiProdottoServizio(jQuery("#hdnProductId" + pid).val());
            var netprice = parseUserNumber(jQuery('#netPrice' + pid).html());
            if(post_tassa){
                kp_adjustment += netprice;
                jQuery("#netPrice" + pid).html(formatUserNumber(0));
                jQuery("#netPriceInput" + pid).val(0);
            }
            else{
                netTotal += netprice;
            }
...
//Retrieve all the tax values for the currently selected product
	...
	new Ajax.Request(
		...
		{queue: {position: 'end', scope: 'command'},
			method: 'post',
			postBody: 'module='+lineItemType+'&action='+lineItemType+'Ajax&file=InventoryTaxAjax&productid='+jQuery("#hdnProductId"+curr_row).val()+'&curr_row='+curr_row+'&productTotal='+jQuery('#totalAfterDiscount'+curr_row).html(),
			onComplete: function(response)
			{
				jQuery("#tax_div"+curr_row).html(response.responseText);
	...
	if (discount_checks[0].checked == true) {
		...
		for (i=0; i<discount_percentage_final_values.length; ++i) {
			var discountPerc = parseUserNumber(discount_percentage_final_values[i]),
				thisDiscount = roundValueFloat(total_tmp * discountPerc / 100.0 );
			// check validity
...
function calcGroupTax() {
    ...
    var dati_tasse = KpDatiTasse(id_configurazione);
	/* kpro@bid291120171400 end*/
	/* kpro@tom150620181140 migrazione vte18.05 end */
	...
	for (var i=1; i<=group_tax_count; ++i) {
        ...
        var group_tax_current_name = KpNomeTassaCorrente("group_tax_percentage" + i);
        
        var aggiungi_a_totale = dati_tasse[group_tax_current_name].aggiungi_a_totale;
        var calcola_su_totale_e_tasse = dati_tasse[group_tax_current_name].calcola_su_totale_e_tasse;

        if (calcola_su_totale_e_tasse == '1' || calcola_su_totale_e_tasse == 1) {
            var tax_amount = roundValueFloat(totale_netto_con_tasse * group_tax_percentage / 100.0);
            //console.log(" - "+group_tax_current_name+": "+totale_netto_con_tasse+" * "+group_tax_percentage+" / 100 = "+tax_amount);
        } else {
            var tax_amount = roundValueFloat(net_total_after_discount * group_tax_percentage / 100.0);
            //console.log(" - "+group_tax_current_name+": "+net_total_after_discount+" * "+group_tax_percentage+" / 100 = "+tax_amount);
        }

        if (aggiungi_a_totale == '1' || aggiungi_a_totale == 1) {
            totale_netto_con_tasse += tax_amount;
        }
		/* kpro@bid291120171400 end */
		/* kpro@tom150620181140 migrazione vte18.05 end */
...
/* kpro@tom150620181140 migrazione vte18.05 */
/* kpro@bid291120171400 */
function KpNomeTassaCorrente(id_tassa) {

    var group_tax_current_html_name = jQuery("#" + id_tassa).prop("name");
    var array_group_tax_current_html_name = group_tax_current_html_name.split("_");

    return array_group_tax_current_html_name[0];
}

function KpGetConfigurazioneDaBusinessUnit(business_unit) {

    var id_configurazione = '0';
    ...
    jQuery.ajax({
        url: 'include/kp_utils/KpGetConfigurazioneDaBusinessUnit.php',
        dataType: 'json',
        data: dati_tassa,
        async: false,
        success: function(data) {
            //console.log(data);
            if (data.length > 0) {
                id_configurazione = data[0].id_configurazione;
            }
        },
        fail: function() {
            console.log("Errore nel recuperare id configurazione");
        }
    });
...
function KpDatiTasse(id_configurazione) {
    ...
    jQuery.ajax({
        url: 'include/kp_utils/KpDatiTasse.php',
        dataType: 'json',
        data: dati_tasse,
        async: false,
        success: function(data) {
        ...
        fail: function() {
            console.log("Errore nel controllare la tassa");
        }
    });
...
function InventorySelectAll(mod,image_pth) {
	...
	function addFromRecordId(entityid, rowid, rawjson) {
		//crmv@32840
		var prod_array = JSON.parse(rawjson || $('popup_product_'+entityid).attributes['vt_prod_arr'].nodeValue, function (key, value) {
		    if (key == 'desc') {
		...
		if(mod!='PurchaseOrder') {
			...
			set_return_inventory(prod_id,prod_name,unit_price,qtyinstk,taxstring,parseInt(row_id),desc,subprod_ids,prod_code);	//crmv@16267
		} else {
			set_return_inventory_po(prod_id,prod_name,unit_price,taxstring,parseInt(row_id),desc,subprod_ids,prod_code);	//crmv@16267
		}
...
//crmv@29190
function initAutocompleteInventoryRow(rel_module,field,display,icon,module,row_no,autocomplete) {
	if (rel_module == 'Products') {
		initAutocomplete(field,display,encodeURIComponent(productPickList(icon,module,row_no,autocomplete)));
	} else if (rel_module == 'Services') {
		initAutocomplete(field,display,encodeURIComponent(servicePickList(icon,module,row_no,autocomplete)));
	}
}
//crmv@29190e
