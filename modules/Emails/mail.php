...
/**   Function used to send email
  ...
  */
function send_mail($module,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$attachment='',$emailid='',$logo='',$newsletter_params='',&$mail_tmp='',$messageid='',$message_mode='')	//crmv@22700	//crmv@25351
{
	...
	if (method_exists($focusEmails, 'overwriteMailConfiguration')) $focusEmails->overwriteMailConfiguration($mail);
	//crmv@80029e
    ...
    if ($newsletter_params) {
    	...
    	if ($newsletter_params['reply_to'] != '') {
			$mail->ClearReplyTos();
			$mail->AddReplyTo($newsletter_params['reply_to']);
    	}
    	// crmv@151474e
...
function setMailerProperties(&$mail,$subject,$contents,$from_email,$from_name,$to_email,$attachment='',$emailid='',$module='',$logo='')	//crmv@86304
{
	...
	if($attachment == 'some') {
		if (!empty($emailid) && isset($_REQUEST['attach_contentids'])) {
			$contlist = array_filter(explode(',', $_REQUEST['attach_contentids']), create_function('$v', 'return $v !== "" && $v >= 0;'));
			if (count($contlist) > 0) {
	...
	if($attachment == 'all')
	{
		...
		if (!empty($_REQUEST['pdf_attachment'])) {
			$mail->AddAttachment($root_directory.'storage/'.$_REQUEST['pdf_attachment']);
		}

		/* kpro@tom08282018091 */

		if(!empty($_REQUEST['pdf_custom_attachment'])){
			include_once('../../config.inc.php');
			chdir($root_directory);
			require_once('include/utils/utils.php');
			include_once('vtlib/Vtiger/Module.php');
			$Vtiger_Utils_Log = true;
			global $root_directory, $adb, $table_prefix, $default_charset;
			session_start();
				
			$i=0;
			while($_REQUEST['pdf_custom_attachment'][$i]){
				$path = getPDFCustomAttachmentPath($_REQUEST['pdf_custom_attachment'][$i]);                            
				$mail->AddAttachment($root_directory.$path.$_REQUEST['pdf_custom_attachment'][$i]);
				$i++;
			}
		}
...
function addAdvancedAttachments(&$mail, $attachments = array()) {	//crmv@86304
	foreach ($attachments as $att) {
		if ($att['sourcetype'] == 'email') {
			...
			if ($att['altbody']) {
				$mail->AddAltBoundary($att['content'], $att['filename'], $att['charset'], $att['contenttype'], $att['encoding'] ?: "base64", $att['method']);
			} else {
...
function AddStringAttachment($mail, $string, $filename, $encoding = "base64", $type = "application/octet-stream", $contentdisposition = "", $cid = "") {
	...
	$mail->attachment[$cur][0] = $string;
	$mail->attachment[$cur][1] = $filename;
	$mail->attachment[$cur][2] = $filename;
	$mail->attachment[$cur][3] = $encoding;
	$mail->attachment[$cur][4] = $type;
	$mail->attachment[$cur][5] = true; // isString
	$mail->attachment[$cur][6] = $contentdisposition;
	if (empty($cid)) {
		$mail->attachment[$cur][7] = 0;
	} else {
		$mail->attachment[$cur][7] = $cid;
	}
}
...
function setCCAddress($mail,$cc_mod,$cc_val)
{
	...
	if($cc_val != '')
	{
		...
		for($i=0;$i<count($ccmail);$i++)
		{
			...
			if($ccmail[$i] != '' && !in_array($addr, $list)) {
...
function append_mail($mail,$account,$parentid,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$send_mode='Single')
{
	...
	//crmv@157490
	$serverConfigUtils = ServerConfigUtils::getInstance();
	$mail_server_smtp = $serverConfigUtils->getConfiguration('email',array('server'),'server_type',true);
	//crmv@157490e
	...
	try {
		...
		if ($messageFocus->appendMessage($mail,$account,'Sent',$_REQUEST['relation'])) {
			...
			if (!empty($saved_message)) {
				if (!empty($_REQUEST['ModCommentsMethod'])) {
					$messageFocus->saveModComment($saved_message[0],$mail->message_id);
				}
			if (!empty($parentid)) {
					$messageFocus->setRecipients($saved_message[0],$parentid);
				}
...
function save_draft_mail($module,$account,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$attachment='',$emailid='',&$mail='',$parentids='',$recipientids='',$send_mode='Single')	//crmv@22700	//crmv@25351
{
	...
	if ($append_result) {
		...
		if (isset($_REQUEST['draft_id']) && $_REQUEST['draft_id'] != '') {
			...
			$result = $adb->pquery("insert into {$table_prefix}_messages_drafts (id, messagehash, userid) values (?,?,?)",array($_REQUEST['draft_id'],$messageFocus->getMessageHash($mail->message_id, ''),$current_user->id)); // crmv@81338
		}
		$saved_message = $messageFocus->getSavedMessages();
...
function HandleInlineAttachments(&$mail,$dir,$mode = 'send'){
	...
	if (!empty($matches)){
		...
		foreach ($matches as $match){
			$cidname = pathinfo($match[1],PATHINFO_FILENAME);
			if (isset($cids[$match[1]])){
				$cidname = $cidname.$cnt;
			}
...
function getPDFCustomAttachmentPath($pdf_custom_attachment){  
    global $adb, $table_prefix, $default_charset;
    
	$trova_id_pdf = explode("_", $pdf_custom_attachment);
	
    $q_path = "SELECT 
				path 
                FROM {$table_prefix}_attachments
                INNER JOIN {$table_prefix}_crmentity ON crmid = attachmentsid
				WHERE deleted = 0 AND attachmentsid = ".$trova_id_pdf[0];
				
    $res_path = $adb->query($q_path);
    if ($adb->num_rows($res_path) > 0) {

		$path = $adb->query_result($res_path, 0, 'path');
