          ...
          'index.php',
            {queue: {position: 'end', scope: 'command'},
                    ...
                    onComplete: function(response) {
                    	...
                    	if (response.responseText.indexOf("Query Failed") >= 0) {
...
function gshow(argg1,type,startdate,enddate,starthr,startmin,startfmt,endhr,endmin,endfmt,viewOption,subtab,view_filter,is_all_day_event,calWhat,calDescription,calLocation)	//crmv@17001 //crmv@20156
{
	...
	if(type != 'todo' && type!='') {
		...
		jQuery('#wdCalendar').contents().find('#bbit-cal-txtSearch').val(top.wdCalendar.empty_search_str);	//crmv@29190
...
function quickEventClickCallback(params) {
	return function(e) {
		...
		parent.jQuery('#addEvent').css('visibility', 'visible'); //mycrmv@167543 /* kpro@bid050920180945 */
...
function updateStatus(record,status,view,hour,day,month,year,type){
	...
	{
		var OptionData = $('view_Option').options[$('view_Option').selectedIndex].value;
		var view_filter = $('filter_view_Option').options[$('filter_view_Option').selectedIndex].value;
		new Ajax.Request(
                	'index.php',
                	{queue: {position: 'end', scope: 'command'},
                        	method: 'post',
                        	postBody: 'module=Calendar&action=ActivityAjax&record='+record+'&'+status+'&view='+view+'&hour='+hour+'&day='+day+'&month='+month+'&year='+year+'&type=change_status&viewOption='+OptionData+'&subtab=event&ajax=true&view_filter='+view_filter,
                        	onComplete: function(response) {
					if(OptionData == 'listview')
					{
	...
	if(type == 'event')
		...
		var view_filter = $('filter_view_Option').options[$('filter_view_Option').selectedIndex].value;
                new Ajax.Request(
                        ...
                        {queue: {position: 'end', scope: 'command'},
                                method: 'post',
                                postBody: 'module=Calendar&action=CalendarAjax&file=ActivityAjax&ajax=true&n_type=nav&viewOption='+OptionData+url+start+'&subtab='+type+'&view_filter='+view_filter,
                                onComplete: function(response) {
	...
	if(subtab == 'event')
	{
		...
		var view_filter = $('filter_view_Option').options[$('filter_view_Option').selectedIndex].value;
         	new Ajax.Request(
                	...
                	{queue: {position: 'end', scope: 'command'},
                        	method: 'post',
                        	postBody: 'module=Users&action=massdelete&return_module=Calendar&return_action=ActivityAjax&idlist='+id+'&view='+view+'&hour='+hour+'&day='+day+'&month='+month+'&year='+year+'&type=activity_delete&viewOption='+OptionData+'&subtab=event&ajax=true&view_filter='+view_filter,
                        	onComplete: function(response) {
					if(OptionData == 'listview')
		                        {
                                                result = response.responseText.split('####');
...
function getInviteesDetail() {
	new Ajax.Request(
          ...
          {queue: {position: 'end', scope: 'command'},
                  method: 'post',
                  postBody: "module=Calendar&action=CalendarAjax&file=wdCalendar&subfile=ajaxRequests/getInviteesDetail",
                  onComplete: function(response) {
...
function LinkDetails(linktype) {
	new Ajax.Request(
          ...
          {queue: {position: 'end', scope: 'command'},
                  method: 'post',
                  postBody: "module=Calendar&action=CalendarAjax&file=wdCalendar&subfile=ajaxRequests/getLinkDetail&linktype=" + linktype,
                  onComplete: function(response) {
                  	var link = response.responseText;
...
function initAutocomplete(id,display,params) {
