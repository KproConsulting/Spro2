	...
	function getTurboliftRelations($focus, $module, $crmid, $relmodules = array(), $excludeMods = array(), $recursive = false, $panelid = null) {
		...
		if (!empty($related_array)) {
			...
			foreach ($related_array as $header => $info) {
				...
				if (!empty($excludeMods) && in_array($info['related_tabname'], $excludeMods)) {
					
					//kpro@tom260720171724
					if($info["related_tabname"] != "TipiCorso"){
						continue; // crmv@109871
					}
					//kpro@tom260720171724 end
				...
				// crmv@140887
				$relatedImg = resourcever('themes/images/modulesimg/'.$info['related_tabname'].'.png');
				if (file_exists($relatedImg)) {
					$related_array_info[$info['relationId']]['img'] = $relatedImg;
				}
				// crmv@140887e
				//crmv@51605
