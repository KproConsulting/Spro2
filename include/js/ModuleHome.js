...
var ModuleHome = {
	...
	editListViewCounts: function(selectView, module, folderid) {
		...
		//me.checkListViewCounts();	/* kpro@tom150620181140 migrazione vte18.05 */
	...
	buildReportChooser: function(data) {
		...
		jQuery.each(data, function(folderid, folder) {
			if (folder && folder.reports && folder.reports.length > 0) {
				html += '<div style="float:left;padding:10px;width:140px;height:140px;text-align:center"><div><img src="'+resourcever('listview_folder.png')+'" border="0" width="96" style="cursor:pointer" onclick="ModuleHome.clickReportFolder(this, '+folderid+');"/></div><div style="text-align:center">'+folder.foldername+' ('+folder.reports.length+')</div></div>';
			}
		});
	...
	clickReportFolder: function(self, folderid) {
		...
		// TODO: Do it properly with css/tpl
		html += '<div style="margin:8px;font-weight:bold"><img src="'+resourcever('folderback.png')+'" style="cursor:pointer" align="bottom" onclick="ModuleHome.clickReportBack(this);" alt="'+alert_arr.LBL_BACK+'" title="'+alert_arr.LBL_BACK+'" border="0"/>&nbsp;&nbsp;'+folder.foldername+(folder.description ? '<span style="color:#C0C0C0;font-style:italic"> - '+folder.description+'</span>' : '') + '</div>';
