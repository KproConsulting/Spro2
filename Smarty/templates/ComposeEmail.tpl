		...
		<td colspan="3">
			<table cellpadding="0" cellspacing="0" width="100%" class="mailClientWriteEmailHeader level2Bg menuSeparation">
				<tr>
					...
					<td width="30%" style="padding-left: 20px;">{'LBL_COMPOSE'|getTranslatedString:'Messages'}</td>
					<td align="right">
								...
								{if ($element_filename.3|@count gt 0) OR ($smarty.request.attachment != '') OR ($COMMON_TEMPLATE_NAME neq '') OR ($webmail_attachments neq '')}{* crmv@22139 *} {* crmv@23060 *} {* crmv@25554 *}
									<div style="width:100%;height:60px;overflow:auto;margin:10px 0;"> {* crmv@121575 *}
										...
										{if $smarty.request.attachment != ''}
											...
											{if $smarty.request.altriid != ''}
												{assign var=attachRequest1 value=$smarty.request.altriid|@vtlib_purify} 
												{assign var=attachRequest value="--"|explode:$attachRequest1} 
												{foreach item=idRequest from=$attachRequest name=cicloAllegati}
													<tr id="row_custom_{$smarty.foreach.cicloAllegati.iteration}">
														<td width="100%" colspan="2">
															{$idRequest}
															<input type="hidden" value="{$idRequest}" name="pdf_custom_attachment[]">
														</td>
														<td align="right">
															<img src="modules/Emails/plupload/jquery.plupload.queue/img/delete.gif" onClick="delAttachments({$smarty.foreach.cicloAllegati.iteration},true)" alt="Elimina" title="Elimina" style="cursor:pointer;">
														</td>
													</tr>
												{/foreach}
											{/if}
...
<div id="droparea" style="opacity:0;visibility:hidden;"><div class="droparea-text">{'LBL_DROP_FILES_HERE'|getTranslatedString}</div></div>
