...
class InventoryUtils extends SDKExtendableUniqueClass {
	...
	function formatUserNumber($number, $autoTrimDecimals = false) {
		$ds = $this->decimalSeparator;
		$ts = $this->thousandsSeparator;
		$op = $this->outputPrecision;
	...
	*/
	...
	function getAllTaxes($available='all', $sh='',$mode='',$id='') {
		...
		$id_configurazione = $this->KpGetConfigurazioneDaFattura($id); /* kpro@bid291120171400 */ /* kpro@tom150620181140 migrazione vte18.05 */
		...
		if ($mode == 'edit' && $id != '' ) {
			...
			if($sh != '' && $sh == 'sh') {
				$query_taxes = "SELECT taxname,	taxid FROM $tablename";
			}
			else{
				/* kpro@bid291120171400 */
				$query_taxes = "SELECT taxname,
							taxid 
							FROM $tablename
							INNER JOIN kp_settings_tasse ON id_tassa = taxid
							WHERE id_configurazione = '{$id_configurazione}'
							ORDER BY aggiungi_a_totale DESC, calcola_su_totale_e_tasse, percentage DESC";
			}

			$result = $adb->pquery($query_taxes,array());
			...
			if($sh != '' && $sh == 'sh') {
				$query_taxes2 = "SELECT * FROM $tablename  WHERE $where_ids  deleted=0 ORDER BY taxid";
			}
			else{
				/* kpro@bid291120171400 */
				$query_taxes2 = "SELECT *
							...
							INNER JOIN kp_settings_tasse ON id_tassa = taxid";
				if($where_ids != ''){
					$query_taxes2 .= " WHERE ($where_ids  deleted=0) AND id_configurazione = '{$id_configurazione}'";
				}
				else{
					$query_taxes2 .= " WHERE deleted=0 AND id_configurazione = '{$id_configurazione}'";
				}
				$query_taxes2 .= " ORDER BY aggiungi_a_totale DESC, calcola_su_totale_e_tasse, percentage DESC";
			}

			$res = $adb->pquery($query_taxes2, $result_ids);
			...
			if($sh != '' && $sh == 'sh') {
				$query_taxes3 = "SELECT * FROM $tablename $where ORDER BY deleted";
			}
			else{
				/* kpro@bid291120171400 */
				$query_taxes3 = "SELECT *
							...
							INNER JOIN kp_settings_tasse ON id_tassa = taxid";
				if($where != ''){
					$query_taxes3 .= " $where AND id_configurazione = '{$id_configurazione}'";
				}
				else{
					$query_taxes3 .= " WHERE id_configurazione = '{$id_configurazione}'";
				}
				$query_taxes3 .= " ORDER BY deleted, aggiungi_a_totale DESC, calcola_su_totale_e_tasse, percentage DESC";
			}

			$res = $adb->pquery($query_taxes3,array());
			/* kpro@tom150620181140 migrazione vte18.05 end */
	...
	/* kpro@tom150620181140 migrazione vte18.05 */
	/* kpro@bid291120171400 */
	function KpGetConfigurazioneDaFattura($id){
		global $log, $adb, $table_prefix;

		$id_configurazione = '0';

		if($id != ''){
			$q_configurazione = "SELECT bu.kp_conf_tassazione
							...
							INNER JOIN {$table_prefix}_crmentity ent1 ON ent1.crmid = bu.kpbusinessunitid
							WHERE ent.deleted = 0 AND ent1.deleted = 0
							AND inv.invoiceid = ".$id;
			$res_configurazione = $adb->query($q_configurazione);
			if($adb->num_rows($res_configurazione) > 0){
				$id_configurazione = $adb->query_result($res_configurazione, 0, 'kp_conf_tassazione');
	...
	function calcProductTotals($prodinfo) {
		...
		if (!empty($prodinfo['discount_percent'])) {
			...
			foreach ($discounts as $d) {
				$disc_perc = $d/100.0;
				$disc_amount = round($price*$disc_perc, $workingPrecision);
				if ($disc_perc) {
					$result['discounts'][] = array(
...
//			} //crmv@43358e
	...
	function calcInventoryTotals($totalinfo) {
		...
		if (!empty($totalinfo['discount_percent']) && floatval($totalinfo['discount_percent']) != 0 ) { //crmv@65329
			...
			foreach ($discounts as $d) {
				$disc_perc = $d/100.0;
				$disc_amount = round($total*$disc_perc, $workingPrecision);
				if ($disc_perc) {
					$result['discounts'][] = array(
						'percentage' => $disc_perc*100.0,
						'starting_price' => $total,
		...
		if (!empty($totalinfo['taxes']) && count($totalinfo['taxes']) > 0) {
			...
			$id_configurazione = $totalinfo['id_configurazione'];
			$dati_tasse = $this->KpDatiTasse($id_configurazione);
			/* kpro@bid291120171400 end */
			/* kpro@tom150620181140 migrazione vte18.05 end */
			...
			foreach ($totalinfo['taxes'] as $taxname => $taxperc) {
				...
				/* kpro@bid291120171400 */
				$aggiungi_a_totale = $dati_tasse[$taxname]["aggiungi_a_totale"];
				$calcola_su_totale_e_tasse = $dati_tasse[$taxname]["calcola_su_totale_e_tasse"];
		
				if($calcola_su_totale_e_tasse == '1' || $calcola_su_totale_e_tasse == 1){
					$taxAmount = round($totale_netto_con_tasse * $taxPercRound/100, $workingPrecision);
				}
				else{
					$taxAmount = round($total * $taxPercRound/100, $workingPrecision);
				}
				...
				if($aggiungi_a_totale == '1' || $aggiungi_a_totale == 1){
					$totale_netto_con_tasse += $taxAmount;
				}
				/* kpro@bid291120171400 end */
				/* kpro@tom150620181140 migrazione vte18.05 end */
...
//			} //crmv@43358
	...
	/* kpro@tom150620181140 migrazione vte18.05 */
	/* kpro@bid291120171400 */
	function KpDatiTasse($id_configurazione){
		global $log, $adb, $table_prefix;

		$dati_tasse = array();

		$q_dati_tasse = "SELECT tax.taxname,
						...
						WHERE kp.id_configurazione = '{$id_configurazione}'";
		$res_dati_tasse = $adb->query($q_dati_tasse);
		$num_dati_tasse = $adb->num_rows($res_dati_tasse);
		if($num_dati_tasse > 0){
			for($i = 0; $i < $num_dati_tasse; $i++){
				$nome_tassa = $adb->query_result($res_dati_tasse, $i, 'taxname');
				if($nome_tassa == null){
					$nome_tassa = '';
				}

				$aggiungi_a_totale = $adb->query_result($res_dati_tasse, $i, 'aggiungi_a_totale');
				if($aggiungi_a_totale == '' || $aggiungi_a_totale == null){
					$aggiungi_a_totale = '0';
				}

				$calcola_su_totale_e_tasse = $adb->query_result($res_dati_tasse, $i, 'calcola_su_totale_e_tasse');
				if($calcola_su_totale_e_tasse == '' || $calcola_su_totale_e_tasse == null){
					$calcola_su_totale_e_tasse = '0';
				}

				$dati_tasse[$nome_tassa] = array(
					"nome_tassa" => $nome_tassa,
	...
	function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='', $returnTotals=false) {
		...
		for ($i=1; $i<=$tot_no_prod; ++$i) {
			...
			if($_REQUEST["deleted".$i] == 1)
				...
				if(isset($_REQUEST['productDescription'.$i]))
					...
					$post_tasse = $this->KpDatiProdottoServizio($prod_id);

					// update the total price
					if($post_tasse){
						$totale_post_tasse += $prodPrices['price_taxes'];
					}
					else{
						$prodTotal += $prodPrices['price_taxes'];
					}
		...
		$id_configurazione = $this->KpGetConfigurazioneDaBusinessUnit($_REQUEST['kp_business_unit']); /* kpro@bid291120171400 */ /* kpro@tom150620181140 migrazione vte18.05 */
	...
	function KpGetConfigurazioneDaBusinessUnit($id){
		global $log, $adb, $table_prefix;
		...
		if($id != ''){
			$q_configurazione = "SELECT bu.kp_conf_tassazione
							...
							WHERE ent.deleted = 0 AND bu.kpbusinessunitid = ".$id;
			$res_configurazione = $adb->query($q_configurazione);
			if($adb->num_rows($res_configurazione) > 0){
				$id_configurazione = $adb->query_result($res_configurazione, 0, 'kp_conf_tassazione');
				if($id_configurazione == '' || $id_configurazione == null){
	...
	function getDetailAssociatedProducts($module,$focus) {
		...
		$smarty->assign('PRODUCT_DETAILS', $prodDetails);

		// final prices
		$finalDetails = $this->getFinalDetails($module, $focus);
		$smarty->assign('FINAL_DETAILS', $finalDetails[1]['final_details']);
	...
	function processProductSqlRow($row, $index, $options, $module, &$focus) {
		...
		if (!empty($discount_percent)) {
			...
			foreach ($prodPrices['discounts'] as $discInfo) {
				// crmv@48527
				$discount_info_message .= $this->formatUserNumber($discInfo['percentage'])."% ".getTranslatedString('LBL_LIST_OF')." ".$this->formatUserNumber($discInfo['starting_price'])." = ".$this->formatUserNumber($discInfo['amount'])."\\n";
			}
	...
	function getFinalDetails($module, $focus, $record=''){
		...
		if ($focus->mode != 'edit') {
			...
			$tax_details = $this->getAllTaxes('available','','',$record); /* kpro@bid291120171400 */
			/* kpro@tom150620181140 migrazione vte18.05 end */
		...
		if ($taxtype == 'group') {
			...
			$totalinfo['id_configurazione'] = $this->KpGetConfigurazioneDaBusinessUnit($focus->column_fields['kp_business_unit']); /* kpro@bid291120171400 */ /* kpro@tom150620181140 migrazione vte18.05 */
		...
		if ($focus->column_fields['hdnDiscountPercent'] != '0' && !empty($discountPercent)) {
			...
			foreach ($totalPrices['discounts'] as $discInfo) {
				// crmv@48527
				$final_discount_info .= $this->formatUserNumber($discInfo['percentage'])."% {$app_strings['LBL_LIST_OF']} ".$this->formatUserNumber($discInfo['starting_price'])." = ".$this->formatUserNumber($discInfo['amount'])."\\n";
			}
