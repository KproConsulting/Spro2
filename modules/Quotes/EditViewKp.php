...
if (!$processMakerView) {
	if(isset($_REQUEST['potential_id']) && $_REQUEST['potential_id'] !=''){
		...
		$potential_focus = CRMEntity::getInstance('Potentials');
		$potential_focus->id = $potential_id;
		$potential_focus->retrieve_entity_info($potential_id, "Potentials");
		$focus->column_fields['kp_agente'] = $potential_focus->column_fields['kp_agente'];
		$focus->column_fields['kp_business_unit'] = $potential_focus->column_fields['kp_business_unit'];
		/* kpro@bid16112017 end */
		$focus->column_fields['kp_stabilimento'] = $potential_focus->column_fields['kp_stabilimento'];
	...
	if(isset($_REQUEST['account_id']) && $_REQUEST['account_id']!='' && $_REQUEST['record']=='' && is_numeric($_REQUEST['account_id'])){
		...
		/* kpro@bid060220181650 */
		$focus->column_fields['kp_agente'] = $acct_focus->column_fields['kp_agente_rel'];
		$focus->column_fields['kp_business_unit'] = $acct_focus->column_fields['kp_business_unit'];
		$focus->column_fields['mod_pagamento'] = $acct_focus->column_fields['mod_pagamento'];
