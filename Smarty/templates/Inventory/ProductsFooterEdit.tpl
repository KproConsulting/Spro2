...
<tr>
<td colspan="3">
	<input type="button" name="Button" class="crmbutton small create" value="{$APP.LBL_ADD_PRODUCT}" onclick="fnAddProductOrServiceRowNew('{$MODULE}','{$IMAGE_PATH}','Products','{$MODE}');" />
	<input type="button" name="Button" class="crmbutton small create" value="{$APP.LBL_ADD_SERVICE}" onclick="fnAddProductOrServiceRowNew('{$MODULE}','{$IMAGE_PATH}','Services','{$MODE}');" />
</td>
</tr>
	...
	<td class="crmTableRow small lineOnTop" align="right">
		...
		<div class="discountUI" id="discount_div_final">
			...
			<table width="100%" border="0" cellpadding="5" cellspacing="0" class="small">
			   ...
			   <tr>
			   {* crmv@2539m *}
				<td align="left"><input type="radio" name="discount_final" onclick="setDiscount(this,'_final');  calcTotal(); calcGroupTax();" {$FINAL.checked_discount_percentage_final}>&nbsp;<label for="discount_percent_final"> % {$APP.LBL_OF_PRICE}</label>&nbsp;<i class="vteicon md-link md-sm md-text" onclick="vtlib_field_help_show(this,'hdnDiscountPercent');">help</i></td>
				<td align="right"><input type="text" class="small" size="5" id="discount_percentage_final" name="discount_percentage_final" value="{$FINAL.discount_percentage_final}" {$FINAL.style_discount_percentage_final} onChange="setDiscount(this,'_final'); calcGroupTax(); calcTotal();">&nbsp;%</td>
				{* crmv@2539me *}
			   </tr>
			   <tr>
				...
				<td align="right"><input type="text" id="discount_amount_final" name="discount_amount_final" size="5" value="{$FINAL.discount_amount_final|formatUserNumber}" {$FINAL.style_discount_amount_final} onChange="setDiscount(this,'_final');  calcGroupTax(); calcTotal();"></td>
			   </tr>
			</table>
					...
					{foreach item=tax_detail name=group_tax_loop key=loop_count from=$FINAL.taxes}
					   <tr>
						<td align="left" class="lineOnTop">
							...
							<!--<input type="text" class="small" size="5" name="{$tax_detail.taxname}_group_percentage" id="group_tax_percentage{$smarty.foreach.group_tax_loop.iteration}" value="{$tax_detail.percentage|formatUserNumber}" onChange="calcTotal()">&nbsp;% -->
							<input type="text" class="small" size="5" name="{$tax_detail.taxname}_group_percentage" id="group_tax_percentage{$smarty.foreach.group_tax_loop.iteration}" value="{$tax_detail.percentage|formatUserNumber}" onChange="calcGroupTax(); calcTotal();">&nbsp;% <!-- kpro@bid291120171400 aggiunto calcGroupTax(); -->
						...
						<td align="right" class="lineOnTop">
							<input type="text" class="small" size="6" name="{$tax_detail.taxname}_group_amount" id="group_tax_amount{$smarty.foreach.group_tax_loop.iteration}" style="cursor:pointer;" value="{0.00|formatUserNumber}" readonly>
						</td>
					   </tr>
					{/foreach}
...
<tr valign="top">
	...
	<td class="crmTableRow small" align="right">
		<input id="shipping_handling_charge" name="shipping_handling_charge" type="text" class="detailedViewTextBox" style="width:40px" align="right" value="{$FINAL.shipping_handling_charge|formatUserNumber}" onChange="calcSHTax();">
	</td>
</tr>
		...
		(+)&nbsp;<b><a href="javascript:void(0);" onClick="displayCoords(this,'shipping_handling_div','sh_tax_div_title',''); calcSHTax();" >{$APP.LBL_TAX_FOR_SHIPPING_AND_HANDLING} </a></b>
				...
				<div class="discountUI" id="shipping_handling_div">
					...
					{foreach item=tax_detail name=sh_loop key=loop_count from=$FINAL.sh_taxes}
					   <tr>
						<td align="left" class="lineOnTop">
							<input type="text" class="small" size="3" name="{$tax_detail.taxname}_sh_percent" id="sh_tax_percentage{$smarty.foreach.sh_loop.iteration}" value="{$tax_detail.percentage|formatUserNumber}" onChange="calcSHTax()">&nbsp;%
						</td>
						<td align="center" class="lineOnTop">{$tax_detail.taxlabel}</td>
						<td align="right" class="lineOnTop">
...
<tr valign="top">
	...
	<td class="crmTableRow small" align="right">
		<input id="adjustment" name="adjustment" type="text" class="detailedViewTextBox" style="width:40px" align="right" value="{$FINAL.adjustment|abs|formatUserNumber}" onChange="calcTotal();"> {* crmv@113949 *}
	</td>
</tr>
