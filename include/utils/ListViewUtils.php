...
class ListViewUtils extends SDKExtendableUniqueClass {
					...
					//crmv@9433 end
						...
						if(($module == 'Calendar' || $module == 'Emails' || $module == 'Fax' || $module == 'Sms' || $module == 'HelpDesk' || $module == 'Invoice' || $module == 'Leads' || $module == 'Contacts') && (($fieldname=='parent_id') || ($name=='Contact Name') || ($name=='Close') || ($fieldname == 'firstname'))) {
							...
							//crmv@23515e
								...
								if($name=='Contact Name') {
									...
									if(($contact_name != "") && ($contact_id !='NULL')) {
										...
										$value =  "<a class=\"goPanelGoGo\" href='index.php?module=Contacts&action=DetailView&parenttab=".$tabname."&record=".$contact_id."' style='".$P_FONT_COLOR."'>".$contact_name."</a>"; // Armando Luscher 05.07.2005 -> priority -> Desc: inserted style="$P_FONT_COLOR"
									}
								}
								if($fieldname == "firstname") {
									$first_name = textlength_check($adb->query_result($list_result,$i-1,"firstname"));
									$value = '<a class="goPanelGoGo" href="index.php?action=DetailView&module='.$module.'&parenttab='.$tabname.'&record='.$entity_id.'">'.$first_name.'</a>';
								}
								if ($name == 'Close') {
							...
							if($name == 'Contact Name') {
								...
								$value="";
								if(($contact_name != "") && ($contact_id !='NULL'))
									$value ="<a class=\"goPanelGoGo\" href='index.php?module=Contacts&action=DetailView&parenttab=".$tabname."&record=".$contact_id."' style='".$P_FONT_COLOR."'>".$contact_name."</a>";
							}
						} elseif($name == 'Product') {
							...
							if($module == 'Accounts') {
								...
								// Fredy Klammsteiner, 4.8.2005: changes from 4.0.1 migrated to 4.2
								$value = '<a class="goPanelGoGo" href="index.php?module=Accounts&action=DetailView&record='.$account_id.'&parenttab='.$tabname.'" style="'.$P_FONT_COLOR.'">'.$account_name.'</a>'; // Armando Luscher 05.07.2005 -> priority -> Desc: inserted style="$P_FONT_COLOR"
							}
							/*
							//crmv@12035
							...
							*/
							elseif($module == 'Projects') {
								...
								$value = '<a class="goPanelGoGo" href="index.php?module=Accounts&action=DetailView&record='.$account_id.'&parenttab='.$tabname.'">'.$accountname.'</a>'; // Armando Luscher 05.07.2005 -> priority -> Desc: inserted style="$P_FONT_COLOR"
							} else {
								$account_id = $adb->query_result($list_result,$i-1,"accountid");
								$account_name = getAccountName($account_id);
								$acc_name = textlength_check($account_name);
								// Fredy Klammsteiner, 4.8.2005: changes from 4.0.1 migrated to 4.2
								$value = '<a class="goPanelGoGo" href="index.php?module=Accounts&action=DetailView&record='.$account_id.'&parenttab='.$tabname.'" style="'.$P_FONT_COLOR.'">'.$acc_name.'</a>'; // Armando Luscher 05.07.2005 -> priority -> Desc: inserted style="$P_FONT_COLOR"
							}
							...
							$value = '<a class="goPanelGoGo" href="index.php?module=Products&action=DetailView&parenttab='.$tabname.'&record='.$product_id.'">'.textlength_check($product_name).'</a>';
						}
						//crmv@7214
						...
						elseif(($module == 'Quotes' && $name == 'Potential Name') || ($module == 'SalesOrder' && $name == 'Potential Name')) {
							...
							$value = '<a class="goPanelGoGo" href="index.php?module=Potentials&action=DetailView&parenttab='.$tabname.'&record='.$potential_id.'">'.textlength_check($potential_name).'</a>';
						...
						elseif($module =='Sms' && $relatedlist != '' && ($name=='Subject' || $name=='Date Sent'))
						{
							$list_result_count = $i-1;
							$tmp_value = $this->getValue($ui_col_array,$list_result,$fieldname,$focus,$module,$entity_id,$list_result_count,"list","",$returnset,$oCv->setdefaultviewid);
							$value = '<a class="goPanelGoGo" href="index.php?module=Sms&action=DetailView&parenttab='.$tabname.'&record='.$entity_id.'"">'.textlength_check($tmp_value).'</a>';	//crmv@16703
							if($name == 'Date Sent')
							{
	...
	*/
	...
	public function transformValueFromDB($value, $module, $fieldname, $uitype, $focus) {
		...
		} elseif (stristr(html_entity_decode($value), "<a href") === false && $uitype != 8) {
	...
	public function getValue($field_result, $list_result,$fieldname,$focus,$module,$entity_id,$list_result_count,$mode,$popuptype,$returnset='',$viewid='')
	{
		...
		elseif($uitype == '10'){
			...
			if(!empty($parent_id)) {
				...
				$value = "<a class=\"goPanelGoGo\" href='index.php?module=$parent_module&action=DetailView&record=$parent_id' title='$valueTitle'>$displayValue</a>";
			} else {
				$value = '';
			}
		} // END
	...
	//crmv@7216e
		...
		if ($fieldname == $focus->list_link_field) {
			...
			//crmv@47243e
				$value = '<a class="goPanelGoGo" href="index.php?action=DetailView&module='.$module.'&record='.$entity_id.'&parenttab='.$tabname.'">'.$value.'</a>';
		}
		//crmv@29079e+33985e
		$log->debug("Exiting getValue method ...");
		return $value;
	...
	public function getListQuery($module,$where='')
	{
		...
		Case "Events": // crmv@114693
			...
			$query.=" WHERE ".$table_prefix."_crmentity.deleted = 0 $setypeCond AND activitytype not in ('Emails','Fax','Sms') ".$where;
...
PAGIN;
	...
	function getMoreInformationsDiv($recordId,$fieldName,$moreInformations) {
		$value = '&nbsp;<a href="javascript:;"><i class="vteicon md-sm valign-bottom" onmouseout="getObj(\'content_'.$fieldName.'_'.$recordId.'\').hide();" onmouseover="getObj(\'content_'.$fieldName.'_'.$recordId.'\').show();">chat</i></a>
			<div id="content_'.$fieldName.'_'.$recordId.'" class="layerPopup" style="width:300px;z-index:10000001;display:none;position:absolute;" onmouseout="getObj(\'content_'.$fieldName.'_'.$recordId.'\').hide();" onmouseover="getObj(\'content_'.$fieldName.'_'.$recordId.'\').show();">
...
// crmv@57608e
//crmv@31126 crmv@122985
function getEntityId($module, $entityName) {
	global $log, $adb,$table_prefix;
	$log->info("in getEntityId " . $entityName);

	$query = 
		"SELECT en.tabid, en.fieldname, en.tablename, en.entityidfield, f.columnname
		FROM {$table_prefix}_entityname en 
		LEFT JOIN {$table_prefix}_field f ON f.fieldname = en.fieldname AND f.tabid = en.tabid
		WHERE en.modulename = ?";
	...
	if (strpos($fieldsname, ',') !== false) {
		...
		// now get the columns for these fields
		$result2 = $adb->pquery("SELECT fieldname, columnname FROM {$table_prefix}_field WHERE tabid = ? AND fieldname IN (".generateQuestionMarks($fieldlists).")", array($tabid, $fieldlists));
		$cols = array();
		while ($row = $adb->fetchByAssoc($result2, -1, false)) {
			$cols[$row['fieldname']] = $row['columnname'];
		}
		// I need to use the order from the entityname table
		$cols = array_values(array_merge(array_flip($fieldlists), $cols));
		// insert spaces now
		...
		$columns[] = $adb->query_result_no_html($result, 0, 'columnname');
	}
	
	$sqlField = '';
	if (count($columns) > 1) {
		$sqlField = $adb->sql_concat($columns);
	} else {
		$sqlField = $columns[0];
	}
	...
	if ($entityName != '') {
		$sql = 
			...
			FROM $tablename
			INNER JOIN {$table_prefix}_crmentity ON {$table_prefix}_crmentity.crmid = $tablename.$entityidfield
			WHERE {$table_prefix}_crmentity.deleted = 0 AND $sqlField = ?";
		$result = $adb->pquery($sql, array($entityName));
		if ($adb->num_rows($result) > 0) {
			$entityId = intval($adb->query_result_no_html($result, 0, $entityidfield));
